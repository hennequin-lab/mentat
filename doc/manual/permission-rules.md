...
- the user config, normally `$XDG_CONFIG_HOME/mentat/config.json`;
- the extra config selected by `MENTAT_CONFIG`.

The extra config's rules evaluate before user rules. Within one file,
`permission.rules.items` evaluates in array order. The enclosing object has
`"version": 1`; obsolete bare arrays and unknown versions are load errors.
Conversation rules and then fixed product rules follow.
...
Project `.mentat/config.json` and `.mentat/config.local.json` files never
contribute permission rules. Mentat strips those rules and reports an
`ignored_project_rules` config diagnostic. This prevents repository content
from granting itself authority.

Rules are not available through the scalar `mentat config set` interface and the
...
`mentat permission remove RULE_ID` removes a writable user-config rule by its
...
externally confined, and Mentat-enforced execution, or every working directory.
...
- `workspace-exact` or `workspace-under`, with a `workspace_path` object
  containing `root` and `rel`;
...
`external` identifies a user-selected boundary that Mentat cannot verify, and
`direct` identifies no confinement claim. By default, Mentat automatically
...
allows only execution proven to use Mentat's sealed sandbox. It does not allow
...
Host matching is exact. Mentat does not case-fold names, resolve aliases,
...
- path: `type`, `op`, `scope`, and either `workspace_path` with `root` and `rel`
  for `scope:"workspace"`, or `path` for `scope:"outside"` or `"unknown"`;
...
After editing, run `mentat permission list --json` to verify evaluation order,
source, storage location, and the normalized rule encoding. Use
`mentat config show --origins` to confirm that no workspace rule was dropped or
