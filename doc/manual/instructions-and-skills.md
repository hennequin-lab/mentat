...
Mentat has three user-guidance surfaces:
...
- **custom commands** are user-invoked prompt templates such as `/review 42`.

They can shape model behavior, but none grants tools, permissions, sandbox
access, or a turn mode. Those capabilities remain separate host decisions.

## Instruction files

Global instructions live at `AGENTS.md` in the Mentat config home:

```text
$MENTAT_CONFIG_HOME/AGENTS.md
```

Without that override, use `$XDG_CONFIG_HOME/mentat/AGENTS.md` or
`~/.config/mentat/AGENTS.md`.

Mentat canonicalizes the requested cwd and uses that exact directory as both the
workspace root and run directory. It does not search ancestors for `.git`.
That directory contributes at most one project instruction file, in this
precedence:
...
A nested `AGENTS.md` is not active until a run starts with its directory as the
cwd. `mentat debug context` reports nested files as not activated during its
audit scan but does not read or include them.

Project instruction discovery runs only in a trusted workspace. While trust is
unknown or explicitly untrusted, Mentat does not stat, walk, or read project
...
Global instructions are not charged against the project budget. The active
project file is limited by `instructions.project_max_bytes` (default `32768`
bytes). Over-budget content is truncated with a visible warning.
...
```sh
mentat debug context
mentat debug context --json
mentat debug prompt --mode build
```
...
Instruction snapshots are not persisted. Mentat reloads instructions for each
new turn, so an edit made between turns takes effect on the next turn. Debug
commands likewise read a new snapshot.
...
These flags are accepted by `mentat run` and the relevant `mentat debug`
commands.
...
In a trusted workspace, skill roots are searched in this order. The first three
workspace roots are anchored at the exact canonical cwd described above:

1. `.mentat/skills` in the workspace;
2. `.agents/skills` in the workspace;
3. `.claude/skills` in the workspace;
4. `skills` in the Mentat config home;
5. each configured `skills.paths` entry, in order;
6. skills compiled into Mentat.

A relative `skills.paths` entry resolves against the run directory. The first
valid, enabled candidate for a name wins; invalid and root-disabled candidates
do not claim the name. A name in `skills.disabled` is disabled in every root and
does not promote a lower-precedence copy. `mentat skills list` reports every
candidate and the winning origin.
...
At request assembly, Mentat gives the model a budgeted catalog of active skill
...
The catalog is refreshed for each turn, and each `skill` call rechecks the
current files. The tool is installed only when skills are enabled and at least
one skill is active when Mentat starts; if none is active then, adding the first
skill requires restarting Mentat.
...
`skills.catalog_max_bytes` (default `8192`) is the trimming budget for the
always-present catalog, not skill bodies. Non-builtin descriptions are visibly
trimmed or removed when necessary; names and built-in descriptions are never
removed, so the value is not a hard cap when those fixed entries alone exceed
it. `mentat skills list` reports catalog bytes and an approximate token cost,
while each skill reports the estimated cost of loading its body.
...
| `skills.project` | Include `.mentat/skills`. |
...
```sh
mentat skills list
mentat skills list --json
mentat skills show release-check
```
...
Custom commands are user-invoked Markdown prompt templates discovered from the
canonical cwd and config home. Workspace commands require trust and cannot
grant tools, permissions, or sandbox access. Inspect discovery without making a
model request:

```sh
mentat commands list [--json]
mentat commands show NAME [--json]
```

See [Custom commands](custom-commands.md) for discovery order, configuration,
naming, expansion, trust confinement, and frontend behavior.
