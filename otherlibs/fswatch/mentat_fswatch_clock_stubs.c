...
CAMLprim double mentat_fswatch_monotonic_seconds(value unit) {
  (void)unit;
#if defined(__APPLE__)
...
CAMLprim value mentat_fswatch_monotonic_seconds_bytecode(value unit) {
  CAMLparam1(unit);
  CAMLreturn(caml_copy_double(mentat_fswatch_monotonic_seconds(unit)));
}
