...
(* The watcher's [Fswatch.Path.t] is construction-only (no [of_string]), so a
   ...
   compares against the rendering of the events a real scan produces. *)
type expected = { path : string; kind : Event.kind }
...
let project_batch = Option.map (List.map project)

let expected_compare a b =
...
let ev kind path = { path; kind }

let events expected actual =
  equal (list expected_testable)
    (List.sort expected_compare expected)
    (List.sort expected_compare (List.map project actual))
...
let with_temp_dir f =
  let file = Filename.temp_file "mentat-fswatch-" ".tmp" in
  Unix.unlink file;
...
let event_values () =
  ...
  match poll watcher with
  | [ ea; eb ] ->
      is_true ~msg:"a real event equals itself" (Event.equal ea ea);
      is_true ~msg:"distinct paths are unequal" (not (Event.equal ea eb));
      is_true ~msg:"events order by path" (Event.compare ea eb < 0);
      is_true ~msg:"Event.compare is a stable sort key"
        (List.equal Event.equal [ ea; eb ] (List.sort Event.compare [ eb; ea ]))
  | other -> failf "expected two created events, got %d" (List.length other)
...
let polling_respects_ignore_predicate () =
  ...
  with_eio @@ fun ~sw ~clock ->
  let ignore path =
    match Fswatch.Path.to_string path with
...
let polling_ignores_watched_root () =
  with_temp_dir @@ fun root ->
  with_eio @@ fun ~sw ~clock ->
  let ignore path = Fswatch.Path.is_root path in
  let watcher = make ~backend:`Polling ~ignore ~sw ~clock root in
  write_file (path root "a") "a";
  events [] (poll watcher);
...
let close_wakes_polling_next () =
  ...
  equal
    (option (list expected_testable))
    ~msg:"closed watcher returns None" None
    (project_batch (next_timeout ~clock watcher))
...
let watch_delivers_batches_until_stopped () =
  ...
  write_file (path root "a") "a";
  equal expected_testable ~msg:"watch delivers the create batch"
    (ev Event.Created "a")
    (project (stream_take ~clock events));
  stop ();
  write_file (path root "b") "b";
...
let watch_stop_during_callback () =
  ...
  ignore (stream_take ~clock ready);
  write_file (path root "a") "a";
  equal expected_testable ~msg:"first batch is delivered" (ev Event.Created "a")
    (project (stream_take ~clock events));
  write_file (path root "b") "b";
  Eio.Time.sleep clock 0.1;
...
let watch_recovers_after_runtime_scan_error () =
  ...
  equal expected_testable ~msg:"watch delivers changes after the scan recovers"
...
let native_close_during_event_flood () =
  with_temp_dir @@ fun root ->
  with_eio @@ fun ~sw:_ ~clock ->
  (* Each iteration opens a native watcher, floods it with an event, and
     double-closes it. [Fswatch.close] drains and releases the FSEventStream
     synchronously (the C [stop] blocks on the drain and frees the handle), so
     the close-during-flood race — exercised by every iteration — needs no
     forced GC to reclaim native state between rounds. *)
    ...
    [
      ...
      group "native"
        [
          ...
          test ~timeout:30.0 "native close during event flood"
