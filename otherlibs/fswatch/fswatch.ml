...
let log_src = Logs.Src.create "mentat.fswatch" ~doc:"Filesystem watcher"
...
module Path = struct
  (* A root-relative path in canonical rendered form: [root] is ["."] and every
     other value is a ["/"]-joined list of validated components. Construction is
     the only way in — {!add_component} rejects [""], ["."], [".."], and any
     component containing ["/"], ["\\"], or NUL — so a built path can only
     descend and can neither inject a separator nor rise above the root. There is
     exactly one string per logical path, which is what makes {!equal} and
     {!compare} byte operations, and those are what the snapshot diff order and
     the [Map] key depend on. *)
  type t = string
  ...
  let is_valid_component name =
    (not (String.equal name ""))
    && (not (String.equal name "."))
    && (not (String.equal name ".."))
    && String.for_all is_component_char name

  (* [add_component parent name] extends [parent] with directory entry [name].
     The rejected [name] is returned so the caller can render a diagnostic. *)
  let add_component parent name =
    if is_valid_component name then
      Ok (if is_root parent then name else parent ^ "/" ^ name)
    else Error name
...
module Event = struct
  ...
  let compare a b =
    match Path.compare a.path b.path with
  ...
  let pp ppf t = Format.fprintf ppf "%a %a" pp_kind t.kind Path.pp t.path
end
...
external monotonic_seconds : unit -> (float[@unboxed])
  = "mentat_fswatch_monotonic_seconds_bytecode"
    "mentat_fswatch_monotonic_seconds"
[@@noalloc]
        ...
        | Ok () -> (
            ...
            | name ->
                ...
                if count > max_directory_entries then
                  ...
                  begin match add_path_work budget (String.length name) with
                  | Error _ as error -> error
...
let relative_child parent name =
  match Path.add_component parent name with
  | Error name ->
      path_error name
        (Printf.sprintf "%S is not a representable path component" name)
  | Ok child -> Ok child
...
let scan ~root ~ignore ~cancelled =
  ...
  if not (Float.is_finite started) then
    ...
    in
    scan_entry ~budget ~ignore ~depth:0 root Path.root empty_snapshot

type 'a scan_result = Finished of ('a, Error.t) result | Cancelled

let scan_in_systhread ~root ~ignore ~cancelled =
  Eio_unix.run_in_systhread ~label:"mentat-file-watcher-scan" (fun () ->
...
module Signal = struct
  ...
  let await t =
    Eio_unix.run_in_systhread ~label:"mentat-file-watcher-await" (fun () ->
        Mutex.lock t.mutex;
        Fun.protect
...
module Native = struct
  ...
  module Inotify = struct
    external supported : unit -> bool = "mentat_file_watcher_inotify_supported"

    external create : unit -> Unix.file_descr * Unix.file_descr
      = "mentat_file_watcher_inotify_create"

    external add_watch : Unix.file_descr -> string -> int
      = "mentat_file_watcher_inotify_add_watch"

    external rm_watch : Unix.file_descr -> int -> unit
      = "mentat_file_watcher_inotify_rm_watch"

    external wake : Unix.file_descr -> unit = "mentat_file_watcher_inotify_wake"

    external read : Unix.file_descr -> Unix.file_descr -> bool
      = "mentat_file_watcher_inotify_read"
                    ...
                    let rec loop () =
                      match
                        Eio_unix.run_in_systhread
                          ~label:"mentat-file-watcher-inotify-read" (fun () ->
  ...
  module Fsevents = struct
    ...
    external available : unit -> bool = "mentat_file_watcher_fsevents_available"

    external create : string -> float -> (unit -> unit) -> raw
      = "mentat_file_watcher_fsevents_create"

    external stop : raw -> unit = "mentat_file_watcher_fsevents_stop"
...
let validate_root root =
  if String.equal root "" || has_nul root then
    ...
    | real_root -> (
        (* [Unix.realpath] already returns a canonical absolute path — no
           symlinks, no [.]/[..], no repeated or trailing separators — so the
           watcher's root normalization is realpath's semantics plus the
           empty/NUL/relative rejections above. *)
        match Unix.stat real_root with
        | exception ex -> unix_error real_root ex
        | stats ->
            if not (stats.Unix.st_kind = Unix.S_DIR) then
              root_error real_root "root is not a directory"
            else Ok real_root)
...
let check_timing ~caller name value =
  ...
  if not (value > 0.0 && finite) then
    invalid_arg
      (Printf.sprintf "Mentat_fswatch.%s: %s=%g must be positive and finite"
         caller name value)

let make_with_cancel ~scan_cancelled ~sw ~clock ?(backend = `Best)
    ?(poll_interval = 0.25) ?(settle_delay = 0.05) ?(ignore = fun _ -> false)
    ~root () =
  check_timing ~caller:"make" "poll_interval" poll_interval;
  check_timing ~caller:"make" "settle_delay" settle_delay;
  let initialized =
    Eio_unix.run_in_systhread ~label:"mentat-file-watcher-init" (fun () ->
...
let watch ~sw ~clock ?backend ?(poll_interval = 0.25) ?(settle_delay = 0.05)
  ...
  in
  Eio.Fiber.fork_daemon ~sw (fun () ->
      Eio.Switch.run ~name:"mentat-fswatch" (fun watch_sw ->
