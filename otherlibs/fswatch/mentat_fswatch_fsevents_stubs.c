...
typedef enum mentat_fsevents_state {
  MENTAT_FSEVENTS_ACTIVE,
  MENTAT_FSEVENTS_STOPPING,
  MENTAT_FSEVENTS_DRAINED,
} mentat_fsevents_state;

typedef struct mentat_fsevents_t {
  value callback;
...
typedef _Atomic(mentat_fsevents_t *) mentat_fsevents_handle;

#define Fsevents_handle(v) ((mentat_fsevents_handle *)Data_custom_val(v))

static mentat_fsevents_t *take_fsevents(value v_t) {
...
static void stop_fsevents(mentat_fsevents_t *t) {
  if (t->stream != NULL) {
...
static void drain_fsevents(void *context) {
  mentat_fsevents_t *t = context;
  stop_fsevents(t);
  atomic_store_explicit(&t->state, MENTAT_FSEVENTS_DRAINED,
...
static void finalize_fsevents(value v_t) {
  mentat_fsevents_t *t = take_fsevents(v_t);
  if (t == NULL)
    return;

  int expected = MENTAT_FSEVENTS_ACTIVE;
  if (atomic_compare_exchange_strong_explicit(
          &t->state, &expected, MENTAT_FSEVENTS_STOPPING,
          memory_order_acq_rel, memory_order_acquire))
    dispatch_group_async_f(t->drain_group, cleanup_queue(), t,
                           drain_fsevents);

  if (expected == MENTAT_FSEVENTS_DRAINED)
...
static struct custom_operations fsevents_ops = {
    "mentat.file_watcher.fsevents", finalize_fsevents, custom_compare_default,
...
static void fsevents_callback(const FSEventStreamRef streamRef, void *client,
  ...
  (void)eventPaths;
  (void)eventIds;
  mentat_fsevents_t *t = (mentat_fsevents_t *)client;
...
CAMLprim value mentat_file_watcher_fsevents_available(value unit) {
  (void)unit;
  return Val_true;
}

CAMLprim value mentat_file_watcher_fsevents_create(value v_path, value v_latency,
                                                  value v_callback) {
  CAMLparam3(v_path, v_latency, v_callback);
  CAMLlocal1(v_t);

  mentat_fsevents_t *t = caml_stat_alloc(sizeof(mentat_fsevents_t));
  t->callback = Val_unit;
  t->stream = NULL;
  t->queue = NULL;
  ...
  atomic_init(&t->state, MENTAT_FSEVENTS_ACTIVE);
  ...
  t->queue = dispatch_queue_create("mentat.file_watcher.fsevents", NULL);
  if (t->queue == NULL) {
    FSEventStreamRelease(t->stream);
    dispatch_release(t->drain_group);
    caml_stat_free(t);
  ...
  v_t = caml_alloc_custom(&fsevents_ops, sizeof(mentat_fsevents_handle), 0, 1);
...
CAMLprim value mentat_file_watcher_fsevents_stop(value v_t) {
  CAMLparam1(v_t);
  mentat_fsevents_t *t =
  ...
  if (atomic_compare_exchange_strong_explicit(
    ...
    caml_acquire_runtime_system();
  } else if (expected == MENTAT_FSEVENTS_STOPPING) {
    CAMLreturn(Val_unit);
  }

  mentat_fsevents_t *owned = t;
  if (atomic_compare_exchange_strong_explicit(
