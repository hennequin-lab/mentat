...
(* The policy applies to scalars, not bytes: a byte-wise scan mistakes the
   continuation bytes of printable Unicode for C1 controls, and escaping one
   splits the sequence and emits invalid UTF-8. Malformed input is escaped a
   byte at a time, so the output is valid UTF-8 whatever the input was. *)
