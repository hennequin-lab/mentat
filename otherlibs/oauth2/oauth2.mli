...
module Response : sig
  (** Raw HTTP responses and pure OAuth response classification.
      ...
      Interpreters such as [mentat_oauth2_eio] construct {!t} values from
      bounded HTTP responses. Standard request decoders and provider-specific
      callers that use low-level transport functions should classify those
      responses with this module so OAuth error objects take precedence over
...
module Sha256 : sig
  (** The vendored SHA-256 backing the PKCE S256 challenge.

      This library carries its own hash so its protocol core depends on no
      external cryptography. The one internal caller is {!Pkce}; it is exposed
      only so conformance tests can check it against the FIPS 180-4 vectors. *)

  val digest_string : string -> string
  (** [digest_string s] is the raw 32-byte SHA-256 digest of [s]. *)
end
