...
(* Self-contained test helpers (this otherlibs member ships its own regression
   net; these mirror the unit suite's shared helpers). *)
...
let callback () =
  ...
  let authorization_with_redirect_query =
    expect_ok "callback authorization with redirect query"
      (O.Authorization.make ~client ~endpoint
         ~redirect_uri:(Uri.of_string "http://localhost/callback?tenant=mentat")
         ~state:(O.State.of_string "state-1")
         ())
  in
  let queried_code =
    expect_ok "callback keeps redirect query"
      (O.Authorization.callback authorization_with_redirect_query
         (Uri.of_string
            "http://localhost/callback?tenant=mentat&code=code-1&state=state-1"))
  in
  ...
  let authorization_with_duplicate_redirect_query =
    expect_ok "callback authorization with duplicate redirect query"
      (O.Authorization.make ~client ~endpoint
         ~redirect_uri:
           (Uri.of_string
              "http://localhost/callback?tenant=mentat&tenant=mentat")
         ~state:(O.State.of_string "state-1")
         ())
  in
  (match
  ...
  ignore
    (expect_ok "callback duplicate redirect query"
       (O.Authorization.callback authorization_with_duplicate_redirect_query
          (Uri.of_string
             "http://localhost/callback?tenant=mentat&tenant=mentat&code=code-1&state=state-1"))
...
(* The vendored SHA-256 against FIPS 180-4 / NIST vectors: the empty string, the
   one-block "abc", the 56-byte input whose length word pushes padding into a
   second block, and the 112-byte input that is genuinely two blocks. The PKCE
   test above additionally checks the composed BASE64URL(SHA256(...)) path
   against the authoritative RFC 7636 vector. *)
let sha256 () =
  let check input expected =
    equal string
      ~msg:(Printf.sprintf "SHA-256 of a %d-byte input" (String.length input))
      expected
      (to_hex (O.Sha256.digest_string input))
  in
...
let () =
  run "oauth2.core"
    [
      group "sha256" [ test "matches FIPS 180-4 vectors" sha256 ];
