...
(* Emits the [Mentat_prompts] module from prompt text files.
...
let skill_entry path =
  ...
  let header =
    ...
    | Ok header -> header
    | Error error -> fail "%s: %s" path (Frontmatter.Error.message error)
  in
  ...
  | `Absent -> fail "%s: frontmatter must carry a description string" path
  | `Not_string -> fail "%s: frontmatter description must be a string" path
  | `String description
  ...
  | `String description -> metadata_text path "description" description);
