  ...
  task "queue-fifo-tests" ~tier:"core" ~category:"tests" ~size:"S"
    ...
    [
      ...
      Eval.Check.gate "kills-lifo-mutation"
        (Eval.Check.shell
           "cp lib/queue_contract.ml /tmp/mentat-eval-queue-contract.ml\n\
            perl -0pi -e 's/let enqueue t x = t @ \\[ x \\]/let enqueue t x = \
            x :: t/' lib/queue_contract.ml\n\
            if dune runtest --root . >/tmp/mentat-eval-queue-mutation.out \
            2>&1; then\n\
           \  cat /tmp/mentat-eval-queue-mutation.out\n\
           \  cp /tmp/mentat-eval-queue-contract.ml lib/queue_contract.ml\n\
           \  exit 1\n\
            else\n\
           \  cp /tmp/mentat-eval-queue-contract.ml lib/queue_contract.ml\n\
           \  dune runtest --root .\n\
