...
module Llm_usage = Mentat_llm.Usage
module Trace = Mentat_eval_trace.Trace
module Trace_metrics = Mentat_eval_trace.Trace_metrics
module Timing = Mentat_eval_trace.Timing
...
let fresh_temp_dir prefix =
  let rec attempt budget =
    if budget = 0 then
      invalid_arg ("mentat-eval: cannot create temp dir for " ^ prefix)
    else
...
let capture_command ~name argv =
  let tmp = Filename.temp_file ("mentat-eval-" ^ name) ".txt" in
  let err = Filename.temp_file ("mentat-eval-" ^ name) ".err" in
  let result =
...
let default_mentat_bin () =
  let ( / ) = Filename.concat in
  let local = Sys.getcwd () / "_build" / "default" / "bin" / "main.exe" in
  if Sys.file_exists local then local else "mentat"

(* Cost interpretation backed by the built-in provider catalog.

   The next-stack built-in catalog is only assembled behind an effectful
   provider runtime, so it is not yet re-exposed as a pure value the eval binary
   can consult without an event loop. The catalog is therefore empty here and
   every cost lookup returns [None]; report cost columns render as absent.
   Restores when the built-in catalog is surfaced as a pure value. *)

let catalog = lazy (Catalog.make [])

let llm_usage_of_eval (usage : Eval.Usage.t) =
  Llm_usage.make ~input:usage.Eval.Usage.input ~output:usage.Eval.Usage.output
    ~reasoning:usage.Eval.Usage.reasoning
    ~cache_read:usage.Eval.Usage.cache_read
    ~cache_write:usage.Eval.Usage.cache_write ()

let model_cost ~model usage =
  match Catalog.resolve (Lazy.force catalog) model with
  | Error _ -> None
  | Ok declared -> Model.cost declared (llm_usage_of_eval usage)
...
module Agent = struct
  ...
  let v ~name run =
    if String.is_empty name then
      invalid_arg "mentat-eval.Agent.v: name must not be empty";
    { name; run }
...
let git_baseline workspace artifact_dir =
  ...
  if not (process_success init.status) then
    ...
    append_line git_exclude ".mentat/";
...
let sample_mean samples =
  match samples with
  | [] -> invalid_arg "mentat-eval: sample mean of empty list"
  | _ :: _ ->
      List.fold_left
...
(* The mentat adapter: drives [mentat run start --json] and reads the metrics
   member of the final JSONL event. The task prompt is passed verbatim.

   The subject environment is constructed, not inherited: every MENTAT_*
...
let mentat_env_drop name = String.starts_with ~prefix:"MENTAT_" name

let real_config_home () =
  match Sys.getenv_opt "MENTAT_CONFIG_HOME" with
...
let mentat_run ~bin ~model ctx ~prompt =
  let artifact path fallback =
    match ctx.Agent.artifact_dir with
    | None -> Filename.temp_file "mentat-eval-agent" fallback
  ...
  let env_extra =
    ctx.Agent.env_extra
    @ [ ("MENTAT_DATA_HOME", data_home); ("MENTAT_CONFIG_HOME", config_home) ]
  in
  (* The old adapter also pinned the permission and sandbox modes and a
     max-steps ceiling on the command line; those flags do not exist on the
     current [run start] command, so the subject runs with default confinement
     and no step cap. Non-interactive confinement and a step ceiling restore
     when those knobs return (as flags or seeded config). *)
  let args =
    [ bin; "run"; "start"; "--json"; "--cwd"; ctx.Agent.workspace ]
    ...
    Fun.protect
      ...
      (fun () ->
        run_process ?timeout_s:ctx.Agent.timeout_s ~env_extra
          ~env_drop:mentat_env_drop ~timing_path ~cwd:(Sys.getcwd ())
  ...
  if result.timed_out then
    ...
    | Some json -> (
          ...
          | Some "turn.finished" -> (
              ...
              | Some outcome ->
                  failed_status Eval.Result.Agent
                    ("mentat turn finished with outcome: " ^ outcome)
              | None ->
                  failed_status Eval.Result.Agent
                    "turn.finished event is missing outcome")
          (* A blocked turn surfaces as [session.waiting] in the current run
             JSONL: the agent is waiting on a decision it cannot resolve
             unattended. *)
          | Some "session.waiting" -> Agent.Outcome.Blocked
          | Some "session.failed" ->
              failed_status Eval.Result.Agent
                (Option.value
                   (json_string_member "message" json)
                   ~default:"mentat run failed")
          | Some other ->
              failed_status Eval.Result.Agent
        ...
        in
        (* The current run JSONL carries no per-run metrics object: the rich
           usage / tool-event stream restores with the run-JSONL wave. When a
           [metrics] object is present it is decoded; when absent the outcome
           records no usage or tool counters rather than failing the run. *)
        match
          Option.bind
            (json_member "metrics" json)
            (json_decode Mentat_session.Metrics.jsont)
        with
        | None -> Agent.Outcome.make ~status ~duration_s ?log ()
        | Some metrics ->
            Agent.Outcome.make ~status
              ~usage:(usage_of_llm metrics.Mentat_session.Metrics.usage)
              ~turns:metrics.Mentat_session.Metrics.turns
              ~tool_calls:metrics.Mentat_session.Metrics.tool_calls
              ~tool_failures:metrics.Mentat_session.Metrics.tool_failures
              ~tool_rejections:metrics.Mentat_session.Metrics.tool_rejections
              ~duration_s ?log ())
...
(* The cmd adapter runs an arbitrary shell command in the workspace with the
   prompt in MENTAT_EVAL_PROMPT. It reports no model metrics; it exists to
...
let cmd_run command ctx ~prompt =
  ...
  let result =
    run_shell ?timeout_s:ctx.Agent.timeout_s
      ~env_extra:(("MENTAT_EVAL_PROMPT", prompt) :: ctx.Agent.env_extra)
...
type agent_kind = Mentat | Noop | Claude | Codex | Cmd of string
...
let agent_kind_of_string raw =
  ...
  | _ ->
      Error
        (`Msg
           (Printf.sprintf
              "unsupported agent %S; supported: mentat, claude, codex, noop, \
               cmd:COMMAND"
              raw))

let pp_agent_kind ppf = function
  | Mentat -> Format.pp_print_string ppf "mentat"
...
let make_agent kind ~mentat_bin ~model =
  match kind with
  | Mentat ->
      ( Agent.v ~name:"mentat" (mentat_run ~bin:mentat_bin ~model),
        command_version mentat_bin )
...
(* Judge: each sample is one [mentat run] call with the judge model in a fresh
...
let judge_sample ~mentat_bin ~judge_model ~artifact_dir ~counter ~prompt =
  incr counter;
  let dir =
  ...
  (* The old judge run used [--ephemeral] (no persistence) and [--max-steps 3];
     neither flag exists on the current [run start] command, so the judge call
     persists a session to the configured data home and runs uncapped. Both
     restore when those flags return. *)
  let result =
  ...
  if result.timed_out || not (process_success result.status) then None
    ...
    @@ fun json ->
    match (json_string_member "type" json, json_string_member "text" json) with
    | Some "turn.finished", Some text -> parse_judge_reply text
    | _ -> None

let make_judge ~mentat_bin ~judge_model ~judge_samples ~artifact_dir
    ~task_prompt ~diff =
      ...
      Some
        (fun ~criterion ->
          ...
          List.init judge_samples (fun _ ->
              match
                judge_sample ~mentat_bin ~judge_model ~artifact_dir ~counter
                  ~prompt
              with
              | sample -> sample
              | exception _ -> None)
...
(* Nothing the harness introduces into the agent-visible surface may carry an
   ...
   developer's own environment, which any local mentat run would see. *)
...
let run_one ~result_dir ~rows_path ~env_extra ~agent ~agent_version ~model
    ~mentat_bin ~mentat_version ~judge_model ~judge_samples task run_index =
      ...
      | Ok () -> (
          ...
          | Ok () -> (
              ...
              | Ok () ->
                  ...
                  in
                  let judge =
                    make_judge ~mentat_bin ~judge_model ~judge_samples
...
let report_command =
  ...
  let warn_missing_rates rows =
    ...
    List.iter
      (fun model ->
        stderr_printf
          "mentat-eval: no catalog rates for model %s; its cost columns \
           are            omitted\n"
          model)
      missing
  in
...
let run_command =
  let agent =
    CArg.(
      value
      & opt (conv (agent_kind_of_string, pp_agent_kind)) Mentat
      & info [ "agent" ] ~docv:"AGENT"
          ~doc:"Agent adapter: mentat, claude, codex, noop, or cmd:COMMAND.")
  in
  let model =
    CArg.(
      value
  ...
  let judge_model =
    CArg.(
      ...
      & info [ "judge-model" ] ~docv:"MODEL"
          ~doc:
            "Judge quality checks with this model via mentat run. Quality \
  ...
  let run suite agent_kind model runs task_filter judge_model judge_samples
      output =
    if runs < 1 then (
      stderr_printf "mentat-eval: --runs must be positive\n";
      CCmd.Exit.some_error)
    else if judge_samples < 1 then (
      stderr_printf "mentat-eval: --judge-samples must be positive\n";
      CCmd.Exit.some_error)
    else
      ...
      in
      if unknown <> [] then (
        stderr_printf "mentat-eval: unknown task ids: %s\n"
          (String.concat ", " unknown);
        CCmd.Exit.some_error)
      else
        let result_dir = make_result_dir output in
        let rows_path = Filename.concat result_dir "rows.jsonl" in
        let mentat_bin = default_mentat_bin () in
        let mentat_version = current_git_rev () in
        let env_extra = ocaml_toolchain_env () in
        let agent, agent_version = make_agent agent_kind ~mentat_bin ~model in
        List.iter
          (fun task ->
            for run_index = 0 to runs - 1 do
              let row =
                run_one ~result_dir ~rows_path ~env_extra ~agent ~agent_version
                  ~model ~mentat_bin ~mentat_version ~judge_model ~judge_samples
                  task run_index
              in
              stdout_printf "%a\n" Eval.Result.pp row
            done)
...
let analyze_run dir =
  match
    Jsont_bytesrw.decode_string Mentat_session.jsont
...
let analysis_markdown ~result_dir ~rows ~analyzed ~skipped ~errors =
  ...
  if skipped > 0 then
    add
      "\n\
       %d run(s) had no session.json and were skipped (non-mentat adapters).\n"
      skipped;
  (match errors with
  | [] -> ()
  | _ ->
...
let analyze_command =
  ...
  let run result_dir =
    if not (Sys.file_exists result_dir && Sys.is_directory result_dir) then (
      stderr_printf "mentat-eval: not a directory: %s\n" result_dir;
      CCmd.Exit.some_error)
    else
      let rows =
...
let command =
  let man =
    [
      ...
      `P
        "mentat-eval runs and reports on the mentat evaluation corpus: it \
  ...
  in
  CCmd.group
    (CCmd.info "mentat-eval" ~doc:"Run and report mentat evaluations." ~man
       ~exits)
    [
      list_command;
      run_command;
