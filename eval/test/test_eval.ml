...
let series ?(task = "task") ?(agent = "mentat") ?(model = Some "test") () =
  {
    Eval.Result.task;
    agent =
      {
    ...
    judge_model = None;
    mentat_version = Some "test-mentat";
  }

let make_result ?(task = "task") ?(agent = "mentat") ?(model = Some "test")
