...
let invalid fn message =
  invalid_arg ("Mentat_eval.Result." ^ fn ^ ": " ^ message)
...
let validate_series series =
  ...
  non_empty_option "make" "mentat_version" series.mentat_version
...
let series_jsont =
  let make task agent judge_model mentat_version =
    decode_invalid_arg (fun () ->
        let series = { task; agent; judge_model; mentat_version } in
        validate_series series;
        series)
  in
  ...
  |> Jsont.Object.opt_mem "judge_model" Jsont.string ~enc:(fun (t : series) ->
      t.judge_model)
  |> Jsont.Object.opt_mem "mentat_version" Jsont.string
       ~enc:(fun (t : series) -> t.mentat_version)
  |> Jsont.Object.error_unknown |> Jsont.Object.finish
