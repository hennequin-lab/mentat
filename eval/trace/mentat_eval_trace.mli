...
(** Session-document trace analysis for the eval suite.
    ...
    A persisted mentat session document is the ground truth of one agent run: it
    ...
    The library is pure: it consumes an already-decoded {!Mentat_session.t} and
    ...
    I/O and drives no agent. It is deliberately separate from [mentat_eval] so
    the core eval library stays agent-agnostic: rows from agents with no mentat
