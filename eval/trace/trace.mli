...
(** The ordered, usage-attributed view of one session.
    ...
    ({!Mentat_session.Event}): a step is one [Provider_settled] that responded;
    an executed call joins a response tool call to its [Tool_settled] result by
    tool-call id, lowering the settled tool-domain result to the model-visible
    answer; a rejected call is one answered without a claim — a permission
    denial or a directly appended error tool result — carrying the arguments
    from the response. Model-visible {e host} tool calls are never claimed and
    never appended as tool results, so they do not appear as calls. *)
...
val of_session : ?timing:Timing.t -> Mentat_session.t -> t
...
val model : t -> Mentat_llm.Model.t option
...
val reasoning_effort : t -> Mentat_llm.Request.Options.Reasoning_effort.t option
