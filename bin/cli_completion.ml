...
(* Standalone shell completion scripts: cmdliner's generic completion function
   (protocol version 1, spoken by this binary via [--__complete]) specialized to
   mentat, as the cmdliner tool's [tool-completion --standalone] would emit.
   Vendored from cmdliner 2.1.1; the protocol version is checked at runtime by
   the scripts, so skew fails loudly rather than silently. The scripts carry no
   command tree of their own — every command, subcommand, and option is answered
   by [mentat --__complete] at completion time, so a new command needs no
   regenerated script.

   The pwsh script diverges from what the tool emits by one token: upstream
   registers the bare word [_mentat_cmdliner] where a scriptblock is expected,
   and pwsh swallows completer errors, so registration fails silently. This copy
...
if ! declare -F _mentat_cmdliner > /dev/null; then
  _completion_loader _mentat_cmdliner
fi
complete -F _mentat_cmdliner mentat|script}

let zsh_script =
  {script|#compdef mentat
function _mentat_cmdliner {
  local w=("${words[@]}") # Keep words intact for restart completion
...
Register-ArgumentCompleter -Native -CommandName mentat -ScriptBlock $Global:_mentat_cmdliner|script}
...
let shell_arg =
  let shells = [ ("bash", Bash); ("zsh", Zsh); ("pwsh", Pwsh) ] in
  Arg.(
    required
    & pos 0 (some (enum shells)) None
    & info [] ~docv:"SHELL"
        ~doc:
          "Shell to emit a completion script for: $(b,bash), $(b,zsh), or \
           $(b,pwsh).")
...
let cmd =
  let man =
    [
      ...
      `P
        "Prints a self-contained completion script for $(i,SHELL). The script \
         drives this binary's cmdliner completion protocol, so commands, \
         subcommands, and options complete without any external helper.";
      `S Manpage.s_examples;
      `Pre "  mentat completion zsh > ~/.local/share/zsh/site-functions/_mentat";
      `Pre
        "  mentat completion bash > \
         ~/.local/share/bash-completion/completions/mentat";
    ]
  in
  Cmd.v
    (Cmd.info "completion" ~doc:"Print a shell completion script."
       ~docs:Cli_common.s_config ~man ~exits:Cli_common.exits)
    (Exit_status.term Term.(const completion $ shell_arg))
