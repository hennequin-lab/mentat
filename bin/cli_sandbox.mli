...
(** The [sandbox] command group.
    ...
    Both resolve the workspace through {!Composition.resolve_workspace} — no
    engine, no credentials, no session, no mutation — so they run on any
...
val print_run_posture : Composition.t -> unit
(** [print_run_posture t] writes the run-start sandbox posture block to stderr
    ...
    default that the client passes to {!Composition.resolve_workspace}.
