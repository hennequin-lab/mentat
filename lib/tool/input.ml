...
let invalid fn message = invalid_arg ("Mentat_tool.Input." ^ fn ^ ": " ^ message)
...
let empty =
  ...
  in
  make codec ~schema:Mentat_llm.Tool.no_input_schema
...
let encode t value =
  match Jsont.Json.encode t.codec value with
  | Ok json -> json
  | Error message -> invalid "encode" ("input re-encoding failed: " ^ message)
