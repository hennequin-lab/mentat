...
let make ?title ?(status = Status.Active) ?forked_from ?delegated_from ~cwd
    ~created_at ~updated_at () =
  check_title "make" title;
  check_times "make" ~created_at ~updated_at;
  (match (forked_from, delegated_from) with
  | Some _, Some _ ->
      invalid "make" "fork and delegation lineage are mutually exclusive"
  | (None | Some _), None | None, Some _ -> ());
  let root = Mentat_workspace.Root.of_dir cwd in
  { title; status; forked_from; delegated_from; root; created_at; updated_at }
...
let pp ppf t =
  Format.fprintf ppf
    "@[<hov>{ title = %a; status = %a; forked_from = %a; delegated_from = %a; \
     cwd = %a; created_at = %a; updated_at = %a }@]"
    (Format.pp_print_option Format.pp_print_string)
    t.title Status.pp t.status
    (Format.pp_print_option Forked_from.pp)
    t.forked_from
    (Format.pp_print_option Delegated_from.pp)
    t.delegated_from Mentat_workspace.Root.pp t.root Time.pp t.created_at
    Time.pp t.updated_at
...
let jsont =
  Jsont.Object.map ~kind:"session metadata"
    (fun title status forked_from delegated_from cwd created_at updated_at ->
