...
(** Locating the OCaml toolchain for spawned tools.
    ...
    Mentat hands the OCaml tools it spawns ([dune], [ocamlmerlin], the compiler)
    ...
    + an explicit override: the [MENTAT_<PROGRAM>] environment variable
      ([MENTAT_DUNE] for [dune]; the program name uppercased, with runs of
...
(** [find t program] is the absolute executable [program] resolves to on the
    ...
    no rung resolves it — including when the [MENTAT_<PROGRAM>] override is set
...
(** [env t ~program] is [t]'s environment, adjusted so an inner shell resolves
    ...
    directory prepended to [PATH] when [MENTAT_<PROGRAM>] is in force; with the
...
val toolchain_env : t -> string array
(** [toolchain_env t] is [t]'s environment with every recovered switch [bin]
    directory ([$OPAM_SWITCH_PREFIX/bin] then [<workspace_root>/_opam/bin],
    those that exist, in ladder order) prepended to [PATH], and unchanged
    (physically) when none was recovered.

    Where {!env} adjusts [PATH] just enough to resolve one program, this bridges
    the whole discovered switch onto the environment [dune] hands its own child
    compiler: a session may resolve [dune] from [PATH] (a global install) while
    the [ocamlc] that [dune] needs lives only in the workspace's local switch. A
    [dune] spawned with {!env}'s unchanged [PATH] then fails to find the
    compiler; a [dune] spawned in this environment finds it, because the switch
    [bin] is on the [PATH] it searches. Recovery here is the same disk-observing
    ladder {!discover} classifies; it never runs [opam env]. *)
