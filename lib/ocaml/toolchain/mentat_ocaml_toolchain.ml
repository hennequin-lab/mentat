...
let prepend_path t dirs =
  match dirs with
  | [] -> t.env
  | dirs ->
      let existing =
        match t.path with None | Some "" -> [] | Some path -> [ path ]
      in
      let path =
        String.concat (String.make 1 path_separator) (dirs @ existing)
      in
      ...
      then
        Array.map
          (fun binding ->
            if String.equal (binding_name binding) "PATH" then entry
            else binding)
          t.env
...
let toolchain_env t = prepend_path t (List.map fst t.recovered)
...
let unreachable_hint t ~program =
  Printf.sprintf
    "%s is not on Mentat's PATH. Mentat inherits the PATH of the process that \
     launched it; a shell may expose %s only through an alias or a hook that \
     child processes do not inherit. Checked: %s. Relaunch Mentat from a shell \
