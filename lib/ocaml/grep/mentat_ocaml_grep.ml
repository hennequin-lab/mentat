...
module Pattern = struct
  ...
  type syntax_error = {
    message : string;
    position : Mentat_ocaml.Position.t option;
  }
  ...
  let position_of_loc loc =
    ...
    Some
      (Mentat_ocaml.Position.make ~line:start.Lexing.pos_lnum
  ...
  let syntax_error_message error =
    ...
    | Some position ->
        Printf.sprintf "%s at line %d, column %d" error.message
          (Mentat_ocaml.Position.line position)
          (Mentat_ocaml.Position.column position)
...
let range_of_compiler_loc (loc : Location.t) =
  let position (p : Lexing.position) =
    Mentat_ocaml.Position.make ~line:p.Lexing.pos_lnum
...
let location_of_compiler_loc ~path (loc : Location.t) =
  Mentat_ocaml.Location.make ~path ~range:(range_of_compiler_loc loc)
...
let search (pattern : Pattern.t) ~path structure =
  ...
  |> List.sort_uniq Mentat_ocaml.Location.compare

(* Metavariable bindings. *)

module Binding = struct
  type captured = Source of Mentat_ocaml.Range.t | Ident of string
...
let search_with_bindings (pattern : Pattern.t) ~path structure =
  ...
  let sorted =
    ...
    |> List.sort (fun (a, _) (b, _) -> Mentat_ocaml.Location.compare a b)
  in
  let rec dedup acc = function
    | [] -> List.rev acc
    | ((loc, _) as entry) :: rest -> (
        match acc with
        | (prev, _) :: _ when Mentat_ocaml.Location.compare prev loc = 0 ->
            dedup acc rest
