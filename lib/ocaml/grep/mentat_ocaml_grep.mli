...
(** Syntactic structural search over OCaml parse trees.
    ...
    [Mentat_ocaml_grep] matches an expression pattern against the untyped
...
module Parse_error : sig
  ...
  val make :
    filename:string ->
    message:string ->
    ?position:Mentat_ocaml.Position.t ->
    unit ->
    t
  ...
  val position : t -> Mentat_ocaml.Position.t option
...
module Binding : sig
  type captured =
    | Source of Mentat_ocaml.Range.t
...
val search_with_bindings :
  ...
  path:Mentat_workspace.Path.t ->
  Parsetree.structure ->
  (Mentat_ocaml.Location.t * Binding.t list) list
