...
(** Library-private construction and decoding helpers shared by every module.
    ...
    Not part of the public {!Mentat_llm} surface. The two decode helpers turn a
    raising smart constructor into a {!Jsont} decoder that re-validates its
    invariants: a decoded value is exactly as checked as a constructed one. *)
...
(** [decode_invalid_arg f] is [f ()], turning an [Invalid_argument] raised by a
    smart constructor into a {!decode_error} so that decoding re-validates every
