...
(** OpenAI client configuration.
    ...
    adapter consumes them when {!Mentat_llm_openai.client} starts requests.

    Construct values with {!make}; pass them to {!Mentat_llm_openai.client}.
...
(** [make ()] is checked OpenAI HTTP configuration.
    ...
    finite, or [max_retries] or [max_stream_retries] is negative. The
    constructor does not validate that [base_url] is an absolute URI or that the
    account-scoping headers identify existing OpenAI resources. *)
