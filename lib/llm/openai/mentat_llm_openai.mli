...
(** OpenAI Responses provider for [mentat.llm].

    This library translates checked {!Mentat_llm.Request.t} values to the OpenAI
    Responses protocol and translates OpenAI responses back to semantic live
...
(** [client ~env ~credential ()] is an OpenAI Responses client.
    ...
    Other models are rejected by {!Mentat_llm.Client.response} before the OpenAI
    ...
    provider-neutral {!Mentat_llm.Error.kind} values; raw response bodies are
    redacted before being attached to {!Mentat_llm.Error.t}.

    Live events expose non-empty text deltas, reasoning-summary deltas, partial
    tool-input deltas, completed tool calls, and usage snapshots. The terminal
    response is produced on [response.completed] or [response.incomplete] and
    contains the durable assistant message, provider response id/model,
    normalized stop reason, provider stop label, usage, and retained reasoning
    parts. Malformed SSE JSON, malformed tool-call input, provider
    [response.failed] or [error] events, terminal events without [response], EOF
    before a terminal event, cancellation during streaming, and streams with no
    assistant parts are returned as stream-phase failures. *)
