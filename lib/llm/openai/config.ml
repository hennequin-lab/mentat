...
let invalid message = invalid_arg ("Mentat_llm_openai.Config.make: " ^ message)
...
let check_max_retries field = function
  | None -> ()
  | Some value -> if value < 0 then invalid (field ^ " must be non-negative")
...
let make ?base_url ?organization ?project ?(headers = [])
    ?(timeout_s = default_timeout_s) ?max_retries ?max_stream_retries () =
