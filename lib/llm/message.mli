...
(** Model-visible messages.
    ...
    Messages are inert transcript facts: no session ids, item ids, timestamps,
    executable tools, permission state, transport handles, or UI state.
    Construct messages with the smart constructors in this module, then use
    {!Transcript} to check order and tool-call/result grammar. Private variants
    are exposed for inspection, not unchecked construction. *)
...
module Assistant : sig
  ...
  module Reasoning : sig
    (** Provider-owned reasoning transcript parts: durable assistant output a
        provider may require back on a later request.

        Two field groups. [text] and [summary] are neutral, human-readable-ish
        content. [id], [encrypted], and [signature] are opaque provider
        continuity state echoed verbatim on replay for round-trip fidelity; llm
        never interprets them. These are model-visible replay facts, unlike
        {!Response.reasoning_summary}, which is log and UI metadata.

        {b Round-trip law.} An adapter replays a reasoning part only if it holds
        the opaque state the provider requires (OpenAI needs [encrypted];
        Anthropic needs [signature]); lacking it, the adapter drops the part
        rather than send unreplayable content. *)
    ...
    val make :
      ...
      unit ->
      t
    (** [make ?id ?summary ?text ?encrypted ?signature ()] is a reasoning part.

        Raises [Invalid_argument] if an optional string is empty, or if no field
        is present. *)

    val id : t -> string option
    (** [id t] is the opaque provider item id, if any. *)
    ...
    (** [signature t] is an opaque provider signature over the reasoning
        content, if any. *)

    val jsont : t Jsont.t
    (** [jsont] maps reasoning parts to and from JSON objects.
  ...
  (** [empty] is an assistant message with no visible text, tool calls, or
      ...
      It represents a successful turn that produced no content, for example a
      content-filter or max-token finish before any output. *)
  ...
  val jsont : t Jsont.t
  (** [jsont] maps assistant messages to and from JSON objects.
...
type t = private
  ...
  | Tool_result of Tool.Result.t
      (** The type for model-visible transcript messages.
          ...
          Instruction strings and content lists are non-empty. Assistant
          messages may be empty through {!Assistant.empty}. Tool-result order is
          checked by {!Transcript}. *)
...
val equal : t -> t -> bool
(** [equal a b] is [true] iff [a] and [b] contain the same durable message data.
    ...
    constructed messages with the same payload compare equal. *)

val jsont : t Jsont.t
(** [jsont] maps messages to and from JSON objects.
