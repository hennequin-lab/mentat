...
(** Provider-boundary errors.
    ...
    consuming a provider stream. They are recoverable data values, suitable for
    session state, logs, and CLI diagnostics. Request-construction errors remain
    local to {!Request.Error}.

    {b Retryability.} This module offers no [transient] predicate. Neither
    {!type:kind} nor {!type:phase} proves whether a request reached the provider
    or incurred work. Provider adapters own retries while they still own that
    protocol evidence; a higher layer resends only under its own durable
    execution policy. *)
...
type phase =
  ...
  | Stream
      (** The type for provider failure phases.
          ...
          [Startup] means request setup failed before the adapter began
          consuming provider output. [Stream] means failure while consuming
          provider output, even if no live {!Event.t} had yet been emitted. *)
...
(** [label kind] is [kind]'s stable lowercase label.
    ...
    Raises [Invalid_argument] if [kind] is an [Other] value that violates the
    documented label invariant. *)
...
val jsont : t Jsont.t
(** [jsont] maps errors to and from JSON objects.
