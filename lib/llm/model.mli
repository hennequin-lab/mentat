...
(** Provider model identities.
    ...
    provider-native model id used for one request. It is an identity, not a
    catalog entry, capability table, price record, credential, or endpoint.
    Capabilities and pricing are added by [mentat.provider], which annotates
    this type rather than redefining it. Provider clients use these identities
    to decide whether a request is accepted. *)

module Api : sig
  (** Provider-local API families. *)
  ...
  *)
  ...
  val jsont : t Jsont.t
  (** [jsont] maps APIs to and from JSON strings.
...
(** [make ~provider ~api ~id] is model [id] interpreted by [provider]'s [api].
    ...
    [id] is the provider-native model id, retained verbatim; it is not parsed or
    normalized here.
...
val jsont : t Jsont.t
(** [jsont] maps models to and from JSON objects.
