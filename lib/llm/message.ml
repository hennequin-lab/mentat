...
let invalid fn message = invalid_arg' "Mentat_llm.Message" fn message
...
module Assistant = struct
  module Reasoning = struct
    ...
    let make ?id ?summary ?text ?encrypted ?signature () =
      ...
      begin match (id, summary, text, encrypted, signature) with
      | None, None, None, None, None ->
          invalid "Assistant.Reasoning.make" "at least one field is required"
      | _ -> ()
      end;
      { id; summary; text; encrypted; signature }
    ...
    let jsont =
      let make id summary text encrypted signature =
        decode_invalid_arg (fun () ->
            make ?id ?summary ?text ?encrypted ?signature ())
      in
  ...
  (* Tool-call parts embed unparsed JSON, so they compare through
     {!Tool.Call.equal}; text and reasoning parts carry only strings. *)
  let part_equal a b =
    match (a, b) with
    | Text a, Text b -> String.equal a b
    | Tool_call a, Tool_call b -> Tool.Call.equal a b
    | Reasoning a, Reasoning b -> a = b
    | (Text _ | Tool_call _ | Reasoning _), _ -> false

  let equal a b = List.equal part_equal a.parts b.parts
...
(* Structural over payload, semantic over embedded tool-call JSON: only the
   assistant case can carry unparsed JSON, and it defers to {!Assistant.equal};
   the other cases hold strings, content, and results free of source metadata. *)
let equal a b =
  match (a, b) with
  | System a, System b | Developer a, Developer b -> String.equal a b
  | User a, User b -> List.equal ( = ) a b
  | Assistant a, Assistant b -> Assistant.equal a b
  | Tool_result a, Tool_result b -> Tool.Result.equal a b
  | (System _ | Developer _ | User _ | Assistant _ | Tool_result _), _ -> false
