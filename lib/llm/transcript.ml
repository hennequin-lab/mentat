...
let invalid fn message = invalid_arg' "Mentat_llm.Transcript" fn message

(* Pending tool calls are tracked incrementally: [start_pending] records the
   calls of an assistant message and [answer_result] folds each result into
   [answered], so appending a message never rescans the whole message list. *)
type pending = {
...
let is_empty t = List.is_empty t.rev_messages
...
let last_assistant_text ?(sep = "") t =
  let rec loop = function
    | [] -> None
    | Message.Assistant assistant :: rest -> (
        match Message.Assistant.texts assistant with
        | [] -> loop rest
        | texts -> Some (String.concat sep texts))
    | ( Message.System _ | Message.Developer _ | Message.User _
      | Message.Tool_result _ )
      :: rest ->
        loop rest
