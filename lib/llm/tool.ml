...
let invalid fn message = invalid_arg' "Mentat_llm.Tool" fn message
...
(* [input_schema] is retained JSON that carries source metadata (text
   locations) when decoded; [Jsont.Json.equal] compares it semantically so a
   decoded declaration and an otherwise-identical constructed one are equal. *)
let equal a b =
  String.equal a.name b.name
  && Option.equal String.equal a.description b.description
  && Jsont.Json.equal a.input_schema b.input_schema
...
module Call = struct
  ...
  (* [input] is unparsed model JSON that carries source metadata (text
     locations) when decoded; [Jsont.Json.equal] compares it semantically so a
     decoded call and an otherwise-identical constructed one are equal. *)
  let equal a b =
    String.equal a.id b.id && String.equal a.name b.name
    && Option.equal String.equal a.signature b.signature
...
module Result = struct
  ...
  let of_raw ~call_id ~name ~error content =
    reject_empty "Result.of_raw" "call_id" call_id;
    reject_bad_name "Result.of_raw" name;
    { call_id; name; content; is_error = error }

  let make ?(error = false) call content =
    of_raw ~call_id:(Call.id call) ~name:(Call.name call) ~error content

  let empty ?error call = make ?error call []
  let text ?error call value = make ?error call [ Content.text value ]

  let with_content t content =
    of_raw ~call_id:t.call_id ~name:t.name ~error:t.is_error content
  ...
  let jsont =
    let make call_id name error content =
      decode_invalid_arg (fun () -> of_raw ~call_id ~name ~error content)
    in
