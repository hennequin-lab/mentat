...
(** Managed local model provider.
    ...
    OpenAI-compatible chat-completions protocol. Mentat's provider-neutral
    ...
    explicit GGUF filesystem paths. *)
...
(** Memory-fit estimation for local models: the machine budget, GGUF header
    inputs, and the fit verdict this provider judges downloads and context by.
*)
...
module Config : sig
  ...
  (** Local inference configuration.
      ...
      [model_dir] defaults to [$XDG_DATA_HOME/mentat/models], or
      [$HOME/.local/share/mentat/models] when [XDG_DATA_HOME] is unset.

      [server_binary] defaults to the [MENTAT_LOCAL_SERVER_BINARY] environment
      ...
      [MENTAT_LOCAL_MEMORY_BUDGET] environment variable when that is set. *)
...
module Manifest : sig
  ...
  val fit : entry -> Modelfit.Model.t
...
module Fit : sig
  ...
  type t = {
    verdict : Modelfit.Verdict.t;  (** The fit verdict. *)
    need_bytes : int;
        (** Estimated memory need at the decisive context: the judged context
            for [Fits] and [Tight], {!Modelfit.min_useful_context} for
            [Wont_run]. *)
  ...
  (** [find id] judges model [id] against the machine budget at
      {!Modelfit.default_context}. [id] is a manifest model, or an existing
      explicit [.gguf] path whose guard inputs are read from the file's own
      header. [None] when [id] is neither, the header cannot be parsed, or the
      machine's memory cannot be determined. The budget comes from
      {!Config.make}'s [memory_budget] when set, otherwise from
      {!Modelfit.Machine.detect}. *)
...
(** [server_binary ()] resolves the inference server binary without running it:
    ...
    Diagnostics such as [mentat doctor] use this to warn before a request fails.
*)
...
module Artifact : sig
  ...
  (** [prepare ~sw ~env ~http ~cancelled id] ensures model [id] is installed.
      ...
      budget cannot run the model even at {!Modelfit.min_useful_context},
      ...
      Cancellation returns a {!Mentat_llm.Error.Cancelled} error. HTTP,
      download, verification, memory-guard, and missing explicit-path failures
      return provider-boundary errors with diagnostic messages. Interrupted and
      failed downloads remove their private candidates; only a complete verified
