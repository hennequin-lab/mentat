...
module Machine = struct
  ...
  let make ~os ~ram_bytes ?wired_limit_bytes () =
    if ram_bytes <= 0 then
      invalid_arg
        "Mentat_llm_local.Modelfit.Machine.make: ram_bytes must be positive";
    (match wired_limit_bytes with
    | Some limit when limit <= 0 ->
        invalid_arg
          "Mentat_llm_local.Modelfit.Machine.make: wired_limit_bytes must be \
           positive"
    | Some _ | None -> ());
    { os; ram_bytes; wired_limit_bytes }

  external sysctl_u64 : string -> int64 = "mentat_modelfit_sysctl_u64"
  ...
  let make ~weights_bytes ~n_kv_layers ~n_kv_heads ~head_dim ~max_context =
    let field name value =
      if value <= 0 then
        invalid_arg
          (Printf.sprintf
             "Mentat_llm_local.Modelfit.Model.make: %s must be positive" name)
    in
...
let estimate ?(kv_dtype = F16) ~context model =
  if context <= 0 then
    invalid_arg "Mentat_llm_local.Modelfit.estimate: context must be positive";
...
let verdict ?(kv_dtype = F16) ?(context = default_context) ~budget model =
  if context <= 0 then
    invalid_arg "Mentat_llm_local.Modelfit.verdict: context must be positive";
...
module Gguf = struct
  ...
  let model ~weights_bytes t =
    if weights_bytes <= 0 then
      invalid_arg
        "Mentat_llm_local.Modelfit.Gguf.model: weights_bytes must be positive";
