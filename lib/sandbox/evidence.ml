...
let equal a b =
  ...
  | Enforced a, Enforced b ->
      Backend.equal a.backend b.backend
      && Mentat_digest.equal a.profile b.profile
...
let pp ppf = function
  ...
  | Enforced { backend; profile } ->
      Format.fprintf ppf "enforced (%s %a)" (Backend.id backend)
        Mentat_digest.pp profile
  ...
  | Enforced { backend; profile } ->
      json_obj
        [
          ...
          ("backend", Jsont.Json.string (Backend.id backend));
          ("profile_hash", Jsont.Json.string (Mentat_digest.to_hex profile));
        ]
  | Refused error ->
      json_obj
        [
