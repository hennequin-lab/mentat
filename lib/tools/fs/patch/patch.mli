...
(** Private parser and exact text applier for apply-patch documents.

    This module turns the model-facing [apply_patch] document format into typed
    operations over root-relative paths and applies parsed update chunks to
    ...
    *** End Patch
    ...
    conflicts, render diffs, create directories, or mutate files. The private
    [apply_patch] adapter should {!parse}, inspect the returned operations,
    resolve each {!Lpath.Rel.t}, observe current file state, apply updates with
    ...
    It is the private parser used by {!Apply_patch}, not part of [mentat.tools]'
    public surface. *)
...
module Error : sig
  type t = private
    ...
    | Invalid_path of { line : int; input : string; error : Lpath.Error.t }
        (** Parse error.
            ...
            [Invalid_path] wraps the {!Lpath.Rel} parser error for a hunk path.
...
module Update : sig
  ...
  *)
  ...
  (** [apply update contents] applies [update] to [contents].
      ...
      [contents] ended in [\n]; inserting parsed lines into an empty string
      therefore returns text without a final newline. [Error e] identifies the
      first chunk that cannot be applied; callers retain the original
      [contents]. *)
end
...
module Operation : sig
  type t = private
    | Add of { path : Lpath.Rel.t; contents : string }
    | Delete of { path : Lpath.Rel.t }
    | Update of {
        path : Lpath.Rel.t;
        move_to : Lpath.Rel.t option;
        update : Update.t;
      }
...
(** [parse text] parses [text] as an apply-patch document.
    ...
    [*** End of File] markers is ignored. A path is the raw trimmed header
    suffix; spaces need no quoting, and quotation marks are ordinary path bytes,
    not delimiters or escapes. A line inside an update chunk that begins with
    space, [+], or [-] is always an update payload line, even if its payload
    looks like a marker. A trailing [\r] at the end of each parsed patch line is
    discarded. Paths are parsed as {!Lpath.Rel.t}; absolute paths and paths that
    escape the root are rejected with {!Error.Invalid_path}.
