...
module File_ref = struct
  ...
  let make ?label path =
    ...
    let label = Option.value label ~default:path |> normalize_utf_8 in
...
module Image_ref = struct
  type t = { media : Mentat_llm.Content.t }

  let make media =
    (match media with
    | Mentat_llm.Content.Media _ -> ()
    | Mentat_llm.Content.Text _ ->
        invalid_arg "Draft.Image_ref.make: expected a media block");
    { media }
  ...
  let media_type t =
    match t.media with
    | Mentat_llm.Content.Media { media_type; _ } -> media_type
    | Mentat_llm.Content.Text _ -> ""

  let equal left right = Mentat_llm.Content.equal left.media right.media
  let pp ppf t = Format.fprintf ppf "{ image = %S }" (media_type t)
end
...
let of_text text =
  let text = normalize_utf_8 text in
    ...
    (fun paste ->
      List.exists
        (function
          ...
          | { element = File_ref _ | Image _; _ } -> false)
        ranges)
    pending
...
let retain_non_overlapping ranges =
  let retain retained range =
    if
      List.exists
        (fun other -> range_overlaps_span range.span other.span)
        retained
    then retained
    else range :: retained
  in
  List.fold_left retain [] ranges |> List.rev
...
let history_paste_ranges text ~blocked pending_pastes =
  let rec loop blocked ranges pending = function
    | [] -> (List.rev ranges, List.rev pending)
    | paste :: rest -> (
        match
          find_unblocked_substring_span text ~start:0 ~blocked
            paste.paste_placeholder
        with
        | None -> loop blocked ranges pending rest
        | Some span ->
            let range =
              { span; element = Paste_placeholder paste.paste_placeholder }
            in
            loop (range :: blocked) (range :: ranges) (paste :: pending) rest)
  in
  loop blocked [] [] pending_pastes

let replace_range span replacement (t : t) =
  let replacement = normalize_utf_8 replacement in
...
let replace_visible_text new_text (t : t) =
  let new_text = normalize_utf_8 new_text in
...
let insert_element visible element t =
  ...
  let span =
    Span.make ~first:(t.cursor - String.length visible) ~last:t.cursor
  in
...
let placeholder_ids text =
  let rec loop index ids =
    if index >= String.length text then ids
    else
      match placeholder_id_at text index with
      | Some id when id > 0 -> loop (index + 1) (id :: ids)
      | Some _ | None -> loop (index + 1) ids
  in
  loop 0 []

(* Placeholder IDs share one namespace across paste and image tokens. Choosing
   the first unused positive integer avoids both collisions and integer overflow
   when visible text contains [max_int]. IDs of fully deleted chunks may be
   reused because their payloads are unreachable. *)
let next_paste_id t =
  let used =
    placeholder_ids (text t)
    @ List.concat_map
        (fun paste -> placeholder_ids paste.paste_placeholder)
        t.pending_pastes
    |> List.sort_uniq Int.compare
  in
  let rec first_unused candidate = function
    | [] -> candidate
    | id :: rest when id < candidate -> first_unused candidate rest
    | id :: rest when id = candidate ->
        if candidate = Int.max_int then
          invalid_arg "Draft.insert_paste: placeholder namespace exhausted";
        first_unused (candidate + 1) rest
    | _ -> candidate
  in
  first_unused 1 used
...
let insert_image image t =
  let id = next_paste_id t in
  let label = Printf.sprintf "[Image #%d]" id in
  insert_element label (Image image) t

let insert_paste ?(char_threshold = large_paste_char_threshold)
    ?(line_threshold = large_paste_line_threshold) pasted t =
  if char_threshold < 0 then
    invalid_arg "Draft.insert_paste: char_threshold must be non-negative";
  if line_threshold <= 0 then
    invalid_arg "Draft.insert_paste: line_threshold must be positive";
  let pasted = pasted |> normalize_paste |> normalize_utf_8 in
  if String.equal pasted "" then t
  else
    ...
    then
      let placeholder =
        paste_placeholder_label ~id:(next_paste_id t) ~newlines
      in
...
let trim_bounds text =
  let length = String.length text in
  let rec loop offset first last =
    if offset = length then
      match first with None -> (length, length) | Some first -> (first, last)
    else
      let decoded = String.get_utf_8_uchar text offset in
      let next = offset + Uchar.utf_decode_length decoded in
      if is_white_space (Uchar.utf_decode_uchar decoded) then
        loop next first last
      else
        let first = Option.value first ~default:offset in
        loop next (Some first) next
  in
  loop 0 None 0

let trim_text text =
  let first, last = trim_bounds text in
  String.sub text first (last - first)

let is_blank t =
  String.equal (trim_text (text (expand_paste_placeholders t))) ""
...
let trim_ranges original trimmed ranges =
  if String.equal trimmed "" then []
    ...
    let trim_start, _ = trim_bounds original in
...
let history_entry (t : t) =
  let file_refs =
    ...
    |> List.filter_map (function
      ...
      | { element = Paste_placeholder _ | Image _; _ } -> None)
  in
  History_entry.make ~file_refs ~pending_pastes:t.pending_pastes (text t)

let of_history_entry entry =
  let entry_text = History_entry.text entry |> normalize_utf_8 in
  let file_ref_ranges =
    History_entry.file_refs entry
    |> List.filter_map (history_file_ref_range entry_text)
    |> retain_non_overlapping
  in
  let history_pastes =
    History_entry.pending_pastes entry
    |> List.map (fun paste ->
        {
          paste_placeholder = normalize_utf_8 paste.paste_placeholder;
          paste_text = normalize_utf_8 paste.paste_text;
        })
  in
  let paste_ranges, pending_pastes =
    history_paste_ranges entry_text ~blocked:file_ref_ranges history_pastes
  in
  let ranges =
    List.sort
...
(* The image media a draft carries, in ascending document order. *)
let submitted_media (t : t) =
  List.sort (fun left right -> Span.compare left.span right.span) t.ranges
  |> List.filter_map (function
    | { element = Image image; _ } -> Some (Image_ref.media image)
    | { element = File_ref _ | Paste_placeholder _; _ } -> None)

(* [t] with every image atom removed and remaining ranges shifted. Image
   placeholders are references, not prompt text, so they never reach the model
   or prompt history; only their media rides {!submitted_media}. Spans are
   removed in descending order so an earlier removal cannot invalidate a later
   span. *)
let without_images (t : t) =
  t.ranges
  |> List.filter_map (function
    | { span; element = Image _ } -> Some span
    | { element = File_ref _ | Paste_placeholder _; _ } -> None)
  |> List.sort (fun left right -> Span.compare right left)
  |> List.fold_left (fun draft span -> replace_range span "" draft) t

let submit (t : t) =
  let media = submitted_media t in
  let base = without_images t in
  let expanded = expand_paste_placeholders base in
  let submitted_text = trim_text (text expanded) in
  if String.equal submitted_text "" && media = [] then None
  else
    let history_text = trim_text (text base) in
    let ranges = trim_ranges (text base) history_text base.ranges in
    let history_draft =
      {
        visible_text = history_text;
        cursor = String.length history_text;
        ranges;
        pending_pastes = pending_pastes_for_ranges base.pending_pastes ranges;
      }
    in
    let history_entry = history_entry history_draft in
    Some
      ( {
          submitted_text;
          submitted_media = media;
          submitted_history_entry = history_entry;
        },
        empty )
