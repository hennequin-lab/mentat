...
module Observed = struct
  ...
  let identity = function
    | Text contents -> Some (Mentat_digest.Content_ref.of_contents contents)
    | Missing | Other -> None
...
module Error = struct
  module Target_error = struct
    type t = Symlink | Not_a_directory

    let message = function
      | Symlink -> "symlink targets are not supported"
      | Not_a_directory -> "not a directory"

    let equal a b =
      match (a, b) with
      | Symlink, Symlink | Not_a_directory, Not_a_directory -> true
      | (Symlink | Not_a_directory), _ -> false
  ...
  let invalid_target ~path reason = Invalid_target { path; reason }
  let workspace ?path error = Workspace (path, error)
  let out_of_workspace path = Out_of_workspace path
  let read_only_path path = Read_only_path path
  ...
  let message = function
    ...
    | Too_large { path; size; max_size } ->
        Format.asprintf "%a: file is too large (%Ld bytes, max %Ld)" W.Path.pp
          path size max_size
    | Invalid_target { path; reason } ->
        Format.asprintf "%a: %a" W.Path.pp path Target_error.pp reason
    ...
    | Out_of_workspace path ->
        Format.asprintf "%a: edit target is no longer inside the workspace"
          W.Path.pp path
    | Read_only_path path ->
        Format.asprintf "%a: edit target belongs to a read-only workspace root"
          W.Path.pp path
    | Protected_path (path, name) ->
        Format.asprintf
  ...
  let equal a b =
    ...
    | ( Invalid_target { path = a_path; reason = a_reason },
        Invalid_target { path = b_path; reason = b_reason } ) ->
        W.Path.equal a_path b_path && Target_error.equal a_reason b_reason
    ...
    | Read_only_path a, Read_only_path b -> W.Path.equal a b
    ...
    | ( ( Invalid_text _ | Duplicate_path _ | State_mismatch _ | Conflict _
        | Too_large _ | Invalid_target _ | Workspace _ | Out_of_workspace _
        | Read_only_path _ | Protected_path _ | Io _ ),
        _ ) ->
        false
...
module Change = struct
  ...
  let default_label path = Textdiff.Label.escaped (W.Path.display path)

  let to_diff ?(label = default_label) t =
    let label = label t.path in
    match (t.before, t.after) with
    | Missing, Text contents -> Textdiff.File_change.create ~label ~contents
    | Text contents, Missing -> Textdiff.File_change.delete ~label ~contents
    | Text before, Text after ->
        Textdiff.File_change.modify ~label ~before ~after
...
module Apply_error = struct
  module Phase = struct
    type t = Preflight | Commit of { target : W.Path.t }

    let equal a b =
      match (a, b) with
      | Preflight, Preflight -> true
      | Commit { target = a }, Commit { target = b } -> W.Path.equal a b
      | (Preflight | Commit _), _ -> false

    let pp ppf = function
      | Preflight -> Format.pp_print_string ppf "Preflight"
      | Commit { target } ->
          Format.fprintf ppf "@[<hov>Commit { target = %a }@]" W.Path.pp target
  end

  type t = { error : Error.t; phase : Phase.t; applied : Result.Entry.t list }
  ...
  let pp ppf t =
    Format.fprintf ppf "@[<hov>{ error = %a; phase = %a; applied = [%a] }@]"
      Error.pp t.error Phase.pp t.phase
      (Format.pp_print_list
         ~pp_sep:(fun ppf () -> Format.pp_print_string ppf "; ")
         Result.Entry.pp)
      t.applied
...
module Apply_evidence = struct
  module Attempt = struct
    type t = { applied : Result.Entry.t list; uncertain : W.Path.t option }
  end

  type apply = Applied of Result.t | Attempted of Attempt.t
  type t = { applies : apply list; observed : W.Path.t list }
end
...
let diff ?label ?limits ?context t =
  Textdiff.render ?limits ?context (List.map (Change.to_diff ?label) t.changes)
...
(* Stopping-phase mint. [preflight_error] is the only mint for failures
   raised before [apply_one] invokes IO [commit]; the [Commit] phase is
   minted exactly where a commit invocation reports an error, so the
   {!Apply_error.Phase} guarantee holds by construction: no code path
   reaches IO [commit] before [apply_checked]. *)
let preflight_error error : Apply_error.t =
  { Apply_error.error; phase = Apply_error.Phase.Preflight; applied = [] }
    ...
    | checked :: rest -> (
        ...
        | Error error ->
            Error
              {
                Apply_error.error;
                phase = Apply_error.Phase.Commit { target = checked.path };
...
let apply_inside_lock ~io ~workspace t =
  ...
  | Error error -> Error (preflight_error error)
  | Ok paths -> (
      match first_duplicate_path paths with
      | Some path -> Error (preflight_error (Error.duplicate_path path))
      | None -> (
...
let apply ~io ~workspace t =
  if is_empty t then Ok Result.empty
    ...
    (* The critical section always returns [Ok], so an [Error] from
       [with_write_lock] means the section never ran: lock acquisition
       failed before any IO, which is a preflight stop. The IO contract
       forbids replacing a completed section's result with a release
       failure. *)
    match
