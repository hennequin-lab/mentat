...
let pp ppf t = Format.pp_print_string ppf (to_string t)
let digest t = Mentat_digest.Content_ref.of_contents (to_string t)
...
module Occurrence = struct
  ...
  let digest t =
    ...
    | Error _ -> Mentat_digest.Content_ref.of_contents t.raw
  ...
  let pp ppf t =
    Format.fprintf ppf "%a:%a:%S" Lpath.Rel.pp t.path Span.pp t.span t.raw
...
module Ref = struct
  ...
  let make ~path ~digest ~ordinal = { path; digest; ordinal }
  let path t = t.path
  ...
  let equal a b =
    ...
    && Int.equal a.ordinal b.ordinal

  let pp ppf t = Format.fprintf ppf "%a#%d" Lpath.Rel.pp t.path t.ordinal

  let jsont =
    Jsont.Object.map ~kind:"cr ref" (fun path digest ordinal ->
        { path; digest; ordinal })
    |> Jsont.Object.mem "path" Codec.rel_path ~enc:path
    |> Jsont.Object.mem "digest" Mentat_digest.Content_ref.jsont ~enc:digest
    |> Jsont.Object.mem "ordinal" Jsont.int ~enc:ordinal
...
module View = struct
  ...
  let equal (a : t) (b : t) =
    Ref.equal a.ref b.ref && Int.equal a.line b.line
    && String.equal a.body b.body
    && Bool.equal a.resolved b.resolved
    && Bool.equal a.malformed b.malformed
    && Option.equal String.equal a.recipient b.recipient

  let pp ppf (t : t) = Format.fprintf ppf "cr %a @@ %d" Ref.pp t.ref t.line

  let jsont =
    Jsont.Object.map ~kind:"cr view"
      (fun ref line body resolved malformed recipient ->
        { ref; line; body; resolved; malformed; recipient })
    |> Jsont.Object.mem "ref" Ref.jsont ~enc:(fun (v : t) -> v.ref)
    |> Jsont.Object.mem "line" Jsont.int ~enc:(fun (v : t) -> v.line)
    |> Jsont.Object.mem "body" Jsont.string ~enc:(fun (v : t) -> v.body)
    |> Jsont.Object.mem "resolved" Jsont.bool ~enc:(fun (v : t) -> v.resolved)
    |> Jsont.Object.mem "malformed" Jsont.bool ~enc:(fun (v : t) -> v.malformed)
    |> Jsont.Object.opt_mem "recipient" Jsont.string ~enc:(fun (v : t) ->
        v.recipient)
    |> Jsont.Object.error_unknown |> Jsont.Object.finish
end

(* The duplicate ordinal keys a serializable {!Ref.t} to one occurrence among
   any that share a path and content digest, so a stale span never rides the
   wire; [views] assigns ordinals in occurrence order and [resolve] walks the
   same order to recover the exact occurrence a ref names. *)
let views occurrences =
  let seen = Hashtbl.create 16 in
  List.map
    (fun occurrence ->
      let path = Occurrence.path occurrence in
      let digest = Occurrence.digest occurrence in
      let key =
        (Lpath.Rel.to_string path, Mentat_digest.Content_ref.to_token digest)
      in
      let ordinal = Option.value (Hashtbl.find_opt seen key) ~default:0 in
      Hashtbl.replace seen key (ordinal + 1);
      let comment = Occurrence.comment occurrence in
      let body =
        match comment with
        | Ok cr -> body cr
        | Error _ -> Occurrence.raw occurrence
      in
      let resolved =
        match comment with
        | Ok cr -> (
      ...
      let recipient =
        match comment with
        | Ok cr -> Option.map Handle.to_string (recipient cr)
        | Error _ -> None
      in
      {
        View.ref = Ref.make ~path ~digest ~ordinal;
        line = Occurrence.line occurrence;
        body;
        resolved;
        malformed = Result.is_error comment;
        recipient;
      })
    occurrences

let resolve_ref occurrences reference =
  let matches occurrence =
    Lpath.Rel.equal (Occurrence.path occurrence) (Ref.path reference)
  ...
  let rec nth remaining = function
    ...
    | occurrence :: rest ->
        if matches occurrence then
          if remaining = 0 then Some occurrence else nth (remaining - 1) rest
        else nth remaining rest
  in
  nth (Ref.ordinal reference) occurrences

let jsont =
  Jsont.map ~kind:"cr comment"
    ~dec:(fun raw ->
...
module Edit = struct
  ...
  let equal a b =
    ...
    | Add a, Add b ->
        Lpath.Rel.equal a.path b.path
        && Int.equal a.line b.line
        && String.equal (to_string a.cr) (to_string b.cr)
    | Replace a, Replace b ->
        Ref.equal a.ref b.ref && String.equal (to_string a.cr) (to_string b.cr)
    | Remove a, Remove b -> Ref.equal a.ref b.ref
    | (Add _ | Replace _ | Remove _), _ -> false

  let pp ppf = function
    | Add { path; line; _ } ->
        Format.fprintf ppf "add(%a@%d)" Lpath.Rel.pp path line
    | Replace { ref; _ } -> Format.fprintf ppf "replace(%a)" Ref.pp ref
    | Remove { ref } -> Format.fprintf ppf "remove(%a)" Ref.pp ref

  let jsont =
    let add_case =
      Jsont.Object.map ~kind:"add edit" (fun path line cr ->
          Add { path; line; cr })
      |> Jsont.Object.mem "path" Codec.rel_path ~enc:(function
        | Add { path; _ } -> path
        | _ -> assert false)
      |> Jsont.Object.mem "line" Jsont.int ~enc:(function
        | Add { line; _ } -> line
        | _ -> assert false)
      |> Jsont.Object.mem "cr" jsont ~enc:(function
        | Add { cr; _ } -> cr
        | _ -> assert false)
      |> Jsont.Object.error_unknown |> Jsont.Object.finish
      |> Jsont.Object.Case.map "add" ~dec:Fun.id
    in
    let replace_case =
      Jsont.Object.map ~kind:"replace edit" (fun ref cr -> Replace { ref; cr })
      |> Jsont.Object.mem "ref" Ref.jsont ~enc:(function
        | Replace { ref; _ } -> ref
        | _ -> assert false)
      |> Jsont.Object.mem "cr" jsont ~enc:(function
        | Replace { cr; _ } -> cr
        | _ -> assert false)
      |> Jsont.Object.error_unknown |> Jsont.Object.finish
      |> Jsont.Object.Case.map "replace" ~dec:Fun.id
    in
    let remove_case =
      Jsont.Object.map ~kind:"remove edit" (fun ref -> Remove { ref })
      |> Jsont.Object.mem "ref" Ref.jsont ~enc:(function
        | Remove { ref } -> ref
        | _ -> assert false)
      |> Jsont.Object.error_unknown |> Jsont.Object.finish
      |> Jsont.Object.Case.map "remove" ~dec:Fun.id
    in
    let cases =
      List.map Jsont.Object.Case.make [ add_case; replace_case; remove_case ]
    in
    let enc_case = function
      | Add _ as e -> Jsont.Object.Case.value add_case e
      | Replace _ as e -> Jsont.Object.Case.value replace_case e
      | Remove _ as e -> Jsont.Object.Case.value remove_case e
    in
    Jsont.Object.map ~kind:"cr edit" Fun.id
