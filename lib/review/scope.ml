...
let of_hunk ~path h =
  Hunk
    {
      path;
      old_start = Textdiff.Hunk.old_start h;
      old_count = Textdiff.Hunk.old_count h;
      new_start = Textdiff.Hunk.new_start h;
      new_count = Textdiff.Hunk.new_count h;
    }
...
let equal a b =
  ...
  | File a, File b -> Lpath.Rel.equal a b
  | Hunk a, Hunk b ->
...
let compare a b =
  match (a, b) with
  | Feature, Feature -> 0
  | File a, File b -> Lpath.Rel.compare a b
  | Hunk a, Hunk b ->
      then_compare (Lpath.Rel.compare a.path b.path) (fun () ->
  ...
  | Line (aside, apath, aline), Line (bside, bpath, bline) ->
      then_compare (Lpath.Rel.compare apath bpath) (fun () ->
          then_compare
...
let contains outer inner =
  ...
  | File outer_path, _ -> (
      ...
      | Some inner_path -> Lpath.Rel.equal outer_path inner_path
      | None -> false)
  | Hunk h, Line (side, line_path, line) -> (
      Lpath.Rel.equal h.path line_path
      &&
      match side with
      | Old ->
...
let pp ppf = function
  ...
  | File path -> Format.fprintf ppf "file %s" (Lpath.Rel.to_string path)
  | Hunk { path; old_start; old_count; new_start; new_count } ->
      Format.fprintf ppf "hunk %s -%d,%d +%d,%d" (Lpath.Rel.to_string path)
        old_start old_count new_start new_count
  | Line (side, path, line) ->
      Format.fprintf ppf "line %s %s:%d"
        (match side with Old -> "old" | New -> "new")
        (Lpath.Rel.to_string path) line
