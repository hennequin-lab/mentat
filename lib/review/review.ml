...
let hunk_evidence hunk =
  ...
  let add_changed marker line =
    ...
    Buffer.add_char buffer
      (if Textdiff.Hunk.Line.newline line then 'n' else 'x');
    Mentat_digest.frame buffer (Textdiff.Hunk.Line.text line)
  in
  List.iter
    (fun line ->
      match Textdiff.Hunk.Line.kind line with
      | Textdiff.Hunk.Line.Context -> ()
      | Textdiff.Hunk.Line.Added -> add_changed '+' line
      | Textdiff.Hunk.Line.Removed -> add_changed '-' line)
...
let line_evidence ~side line =
  ...
  Buffer.add_char buffer (if Textdiff.Hunk.Line.newline line then 'n' else 'x');
  Mentat_digest.frame buffer (Textdiff.Hunk.Line.text line);
  Mentat_digest.Content_ref.of_contents (Buffer.contents buffer)
    ...
    (fun hunk ->
      List.find_opt
        (fun candidate ->
          let number =
            match side with
            | Scope.Old -> Textdiff.Hunk.Line.old_line candidate
            | Scope.New -> Textdiff.Hunk.Line.new_line candidate
          in
...
let hunk_matches_scope (h : Scope.t) hunk =
  ...
  | Scope.Hunk { old_start; old_count; new_start; new_count; _ } ->
      Int.equal (Textdiff.Hunk.old_start hunk) old_start
      && Int.equal (Textdiff.Hunk.old_count hunk) old_count
      && Int.equal (Textdiff.Hunk.new_start hunk) new_start
      && Int.equal (Textdiff.Hunk.new_count hunk) new_count
  | _ -> false
...
let stops t =
  ...
  in
  let in_file path (_, occ) = Lpath.Rel.equal (Cr.Occurrence.path occ) path in
  let file_stops file =
  ...
  let outside =
    List.filter
      (fun (_, occ) ->
        let rel = Cr.Occurrence.path occ in
        not (List.exists (Lpath.Rel.equal rel) feature_paths))
      indexed
  in
  let outside =
    List.sort
      (fun (i, a) (j, b) ->
        let c =
          Lpath.Rel.compare (Cr.Occurrence.path a) (Cr.Occurrence.path b)
        in
...
let carry_mark feature mark =
  ...
  | Scope.Feature ->
      if Mentat_digest.Content_ref.equal (Feature.digest feature) evidence then
        Some mark
      else None
  | Scope.File path -> (
      match Feature.find_file feature ~path with
      | Some file
        when Mentat_digest.Content_ref.equal (Feature.File.digest file) evidence
        ->
          Some mark
      | _ -> None)
  | Scope.Hunk { path; _ } -> (
      ...
      | Some file -> (
          ...
          | Feature.File.Text hunks -> (
              match
                List.filter
                  (fun hunk ->
                    Mentat_digest.Content_ref.equal (hunk_evidence hunk)
                      evidence)
                  hunks
              with
              | [ hunk ] ->
  ...
  | Scope.Line (side, path, line) -> (
      ...
      | Some file -> (
          ...
          | Feature.File.Text hunks -> (
              match find_line hunks ~side ~line with
              | Some found
                when Mentat_digest.Content_ref.equal
...
let preserve_cursor old_t new_t =
  ...
  let kept =
    ...
    | Cursor.Cr index -> (
        ...
        | None -> None
        | Some occ -> (
            let digest = Cr.Occurrence.digest occ in
            let path = Cr.Occurrence.path occ in
            let same candidate =
              Mentat_digest.Content_ref.equal
                (Cr.Occurrence.digest candidate)
                digest
              && Lpath.Rel.equal (Cr.Occurrence.path candidate) path
            in
            let ordinal =
              let count = ref 0 in
  ...
  | None -> (
      let path_hint =
        ...
        | Cursor.Cr index -> Option.map Cr.Occurrence.path (cr old_t index)
      in
...
let open_crs t =
  Array.fold_left
    (fun open_crs occ ->
      match Cr.Occurrence.comment occ with
      | Ok comment -> (
          match Cr.status comment with
          | Cr.Status.Open _ -> open_crs + 1
          | Cr.Status.Resolved _ -> open_crs)
      | Error _ -> open_crs)
    0 t.crs
...
let equal a b =
  ...
  && Array.for_all2 (fun x y -> Cr.Occurrence.equal x y) a.crs b.crs
