...
let invalid fn message = invalid_arg ("Mentat_diagnostic." ^ fn ^ ": " ^ message)
...
let of_text_or ?(hints = []) ~fallback text =
  if String.is_empty text then make ~hints fallback
  else
    match text.[0] with
    | '\n' | '\r' -> make ~context:text ~hints fallback
    | _ -> of_text ~hints text
...
let equal a b =
  ...
  && Option.equal String.equal a.context b.context
  && List.equal String.equal a.hints b.hints
...
let jsont =
  let dec message context hints =
    try make ?context ~hints message
    with Invalid_argument error -> Jsont.Error.msg Jsont.Meta.none error
  in
  Jsont.Object.map ~kind:"Mentat_diagnostic" dec
  |> Jsont.Object.mem "message" Jsont.string ~enc:(fun t -> t.message)
  |> Jsont.Object.opt_mem "context" Jsont.string ~enc:(fun t -> t.context)
  |> Jsont.Object.mem "hints" (Jsont.list Jsont.string) ~dec_absent:[]
       ~enc:(fun t -> t.hints)
       ~enc_omit:(fun hints -> List.is_empty hints)
