...
module Item : sig
  ...
  (** [access item] is the stable permission identity. Display and change do not
      affect policy. *)
...
val make : ?source:string -> ?display:string -> Item.t list -> t
(** [make ?source ?display items] is one atomic permission request for [items].
...
val of_accesses : ?source:string -> ?display:string -> Access.t list -> t
(** [of_accesses ?source ?display accesses] is one atomic permission request for
    [accesses].
...
val jsont : t Jsont.t
(** [jsont] maps requests to version-1 JSON objects.
