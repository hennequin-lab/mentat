...
let invalid fn message = invalid_arg' "Mentat_permission.Access" fn message
...
and command =
  | Shell of { text : string; cwd : path_scope; execution : command_execution }
  | Argv of {
      program : string;
      args : string list;
      cwd : path_scope;
...
and command_execution = Enforced of Command_confinement.t | External | Direct

let unknown_scope path =
  reject_empty "unknown_path" "unknown path" path;
  Unknown path

let workspace_scope path = Workspace path
...
module Path_scope = struct
  ...
  let to_string = function
    | Workspace path ->
        ...
        in
        if Mentat_workspace.Path.is_root path then root_key
        else root_key ^ "/" ^ Mentat_workspace.Path.display path
    | Outside_workspace path -> Lpath.Abs.to_string path
    | Unknown path -> path

  let pp ppf = function
    | Workspace path ->
        Format.fprintf ppf "workspace(%a)" Mentat_workspace.Path.pp path
    | Outside_workspace path ->
        Format.fprintf ppf "outside(%S)" (Lpath.Abs.to_string path)
    | Unknown path -> Format.fprintf ppf "unknown(%S)" path
end

let stable_scope = function
  | Workspace path ->
      let root_key = Mentat_workspace.Path.root_key path in
      let relative = Mentat_workspace.Path.rel path in
      "workspace:"
      ^ stable_field (Mentat_workspace.Root.Key.to_string root_key)
      ^ ":"
      ^ stable_field (Lpath.Rel.to_string relative)
  | Outside_workspace path ->
      "outside:" ^ stable_field (Lpath.Abs.to_string path)
...
let path ~op workspace_path = path_scope ~op (workspace_scope workspace_path)
...
module Command = struct
  ...
  type t = command =
    | Shell of { text : string; cwd : path_scope; execution : execution }
    | Argv of {
        program : string;
        args : string list;
        cwd : path_scope;
  ...
  let execution = function
    | Shell { execution; _ } | Argv { execution; _ } | Code { execution; _ } ->
        execution
  ...
  let confinement_read_jsont =
    Jsont.enum [ ("project", Confinement.Project); ("all", Confinement.All) ]
  ...
  let confinement_network_jsont =
    Jsont.enum
      [
        ("restricted", Confinement.Restricted); ("enabled", Confinement.Enabled);
      ]
  ...
  let stable_text = function
    | Shell { text; cwd; execution } ->
        ...
        ^ ":" ^ stable_field text ^ ":" ^ stable_scope cwd
    | Argv { program; args; cwd; execution } ->
...
(* Identity is the canonical [stable_text] projection, so [equal] and [compare]
   both distinguish exactly what the digest bytes distinguish and cannot silently
   drift from it the way a structural [( = )] or [Stdlib.compare] would: a future
   variant cannot make them disagree, because both ride the one projection. *)
let equal a b = String.equal (stable_text a) (stable_text b)
let compare a b = String.compare (stable_text a) (stable_text b)
...
let decode_abs_path path =
  ...
  | Ok path -> path
  | Error error ->
      decode_error ("invalid absolute path: " ^ Lpath.Error.message error)
...
let path_jsont =
  let make op scope path_value workspace_path =
    match (scope, path_value, workspace_path) with
    | "workspace", None, Some workspace_path -> path ~op workspace_path
    | "outside", Some path_text, None ->
        decode_invalid_arg (fun () ->
            outside_workspace_path ~op (decode_abs_path path_text))
    | "unknown", Some path_text, None ->
        decode_invalid_arg (fun () -> unknown_path ~op path_text)
    | "workspace", _, _ ->
        decode_error
          "workspace path requires workspace_path and must not carry path"
    | "outside", _, _ -> decode_error "outside path requires only path"
    | "unknown", _, _ -> decode_error "unknown path requires only path"
    | scope, _, _ -> decode_error ("unknown path scope: " ^ scope)
  in
  ...
  |> Jsont.Object.opt_mem "workspace_path" Mentat_workspace.Path.jsont
       ~enc:workspace_path
  |> Jsont.Object.error_unknown |> Jsont.Object.finish

let path_scope_jsont =
  let make scope path workspace_path =
    match (scope, path, workspace_path) with
    | "workspace", None, Some workspace_path -> workspace_scope workspace_path
    | "outside", Some path, None ->
        decode_invalid_arg (fun () ->
            outside_workspace_scope (decode_abs_path path))
    | "unknown", Some path, None ->
        decode_invalid_arg (fun () -> unknown_scope path)
    | "workspace", _, _ ->
        decode_error
          "workspace path scope requires workspace_path and must not carry path"
    | "outside", _, _ -> decode_error "outside path scope requires only path"
    | "unknown", _, _ -> decode_error "unknown path scope requires only path"
    | scope, _, _ -> decode_error ("unknown path scope: " ^ scope)
  in
  let scope = function
  ...
  in
  let path = function
    | Outside_workspace path -> Some (Lpath.Abs.to_string path)
...
let custom_jsont =
  let make name subject = decode_invalid_arg (fun () -> custom ?subject name) in
...
let pp ppf = function
  | Path { op; scope } ->
      Format.fprintf ppf "path(%a, %a)" pp_path_op op Path_scope.pp scope
