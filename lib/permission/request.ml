...
module Change = struct
  ...
  let invalid = invalid_arg' "Mentat_permission.Request.Change" "make"
...
module Item = struct
  ...
  let invalid = invalid_arg' "Mentat_permission.Request.Item" "make"
...
let invalid_make = invalid_arg' "Mentat_permission.Request" "make"
let invalid_of_accesses = invalid_arg' "Mentat_permission.Request" "of_accesses"
...
let pp ppf request =
  ...
  let metadata =
    List.filter_map Fun.id
      [
        Option.map
          (fun source -> Printf.sprintf "source=%S" source)
          request.source;
        Option.map
          (fun display -> Printf.sprintf "display=%S" display)
          request.display;
      ]
  in
  match metadata with
  | [] ->
      Format.fprintf ppf "request([%a]%a)" pp_accesses accesses changes
        change_count
  | metadata ->
      Format.fprintf ppf "request(%s, [%a]%a)"
...
let jsont =
  let decode version source display items =
    ...
    decode_invalid_arg (fun () -> make ?source ?display items)
  in
