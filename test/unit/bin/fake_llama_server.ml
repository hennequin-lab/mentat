...
(* A stand-in for llama-server used by test_llm_local. It accepts the argv
   shape Mentat_llm_local passes to the real binary, binds the requested
   ...
   MENTAT_FAKE_LLAMA_SSE. When MENTAT_FAKE_LLAMA_DUMP is set, each chat request
...
let () =
  ...
  record "MENTAT_FAKE_LLAMA_PID_FILE" (string_of_int (Unix.getpid ()));
  if enabled "MENTAT_FAKE_LLAMA_IGNORE_TERM" then
    Sys.set_signal Sys.sigterm
      (Sys.Signal_handle (fun _ -> record "MENTAT_FAKE_LLAMA_TERM_FILE" "term"));
  if enabled "MENTAT_FAKE_LLAMA_EXIT_BEFORE_BIND" then exit 23;
  ...
  let partial_health_count =
    match Sys.getenv_opt "MENTAT_FAKE_LLAMA_PARTIAL_HEALTH_COUNT" with
    | None -> 0
        ...
        | request_line, body ->
            if String.starts_with ~prefix:"GET /health" request_line then begin
              ...
              Option.iter
                ...
                (Sys.getenv_opt "MENTAT_FAKE_LLAMA_HEALTH_DUMP");
              if enabled "MENTAT_FAKE_LLAMA_UNHEALTHY" then
              ...
              String.starts_with ~prefix:"POST /v1/chat/completions"
                request_line
            then begin
              (match Sys.getenv_opt "MENTAT_FAKE_LLAMA_DUMP" with
              | Some path when not (String.equal path "") ->
                  let oc = open_out_bin path in
                  output_string oc body;
                  close_out_noerr oc
              | Some _ | None -> ());
              begin match Sys.getenv_opt "MENTAT_FAKE_LLAMA_SSE" with
              | Some path when not (String.equal path "") ->
              ...
              if enabled "MENTAT_FAKE_LLAMA_EXIT_AFTER_CHAT" then exit 0
            end
