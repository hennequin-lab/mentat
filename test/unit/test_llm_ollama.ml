...
module Ollama = Mentat_llm_ollama
...
let image_user_content_encodes_multimodal () =
  let transcript =
    Llm.Transcript.of_list_exn
      [
        Llm.Message.user
          [
            ...
            Llm.Content.media ~media_type:"image/png" (`Base64 "aGVsbG8=");
          ];
      ]
  in
  let result, requests =
  ...
  let messages = list_field "body" "messages" body in
  equal int ~msg:"message count" 1 (List.length messages);
  let user = List.hd messages in
  equal string ~msg:"user role" "user" (string_field "user" "role" user);
  (* Media in user content lifts [content] from a plain string to a multimodal
     parts array: text as [type:text], image as [type:image_url] with a data
     URL. *)
  let parts = list_field "user" "content" user in
  equal int ~msg:"content parts" 2 (List.length parts);
  let text_part = List.nth parts 0 in
  equal string ~msg:"text part type" "text"
    (string_field "text part" "type" text_part);
  equal string ~msg:"text part text" "what is this"
    (string_field "text part" "text" text_part);
  let image_part = List.nth parts 1 in
  equal string ~msg:"image part type" "image_url"
    (string_field "image part" "type" image_part);
  let image_url = field "image part" "image_url" image_part in
  equal string ~msg:"image data url" "data:image/png;base64,aGVsbG8="
    (string_field "image url" "url" image_url)
...
let unsupported_requests_do_not_touch_transport () =
  ...
  let unsupported =
    [
      ( "non-image media in user content",
        ...
        request
          ...
          () );
      ( "image media in a tool result",
        Llm.Error.Unsupported,
        request
          ~transcript:
            (Llm.Transcript.of_list_exn
               [
                 Llm.Message.user_text "hello";
                 Llm.Message.assistant
                   (Llm.Message.Assistant.make
                      [ Llm.Message.Assistant.tool_call call ]);
...
(* The OpenAI-compatible transport reaches remote gateways as well as a local
   daemon, so it carries the same retry policy as the other providers. The budget
   floors, backoff, and reason mapping are pinned directly against the shared
   loop in test_llm_http; this smoke proves the transport wires that loop — a
   retryable status recovers on a fresh attempt and announces the retry with the
   deepened capacity limit. *)
...
(* A stream that dies before any output surfaced is re-run on the deeper stream
   budget, the same as the Responses and Messages transports. *)
let stream_faults_are_retried () =
  let result, requests =
    with_ollama_server
      (fun index request ->
        ignore request;
        if index = 0 then sse_response "" else text_stream "recovered")
      (fun port ->
        let config =
          Ollama.Config.make
            ~base_url:("http://127.0.0.1:" ^ string_of_int port)
...
(* [max_retries = 0] is the caller's blanket "no retries" and silences the
   stream budget too, so a retryable fault surfaces on the first request. *)
