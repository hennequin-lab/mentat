...
module Openai = Mentat_llm_openai
...
let with_openai_server respond f =
  ...
  in
  let requests_path = Filename.temp_file "mentat-openai-requests" ".bin" in
...
let run_stream ?(cancelled = fun () -> false) ?config
  ...
  in
  let client = Openai.client ~env ~config ~credential () in
  let events = ref [] in
  let observe event =
...
let model_and_config_contracts () =
  ...
  equal (option int) ~msg:"max stream retries" (Some 7)
    (Openai.Config.max_stream_retries config);
  equal (option int) ~msg:"max stream retries unset by default" None
    (Openai.Config.max_stream_retries Openai.Config.default);
  ...
  expect_invalid_arg "max_stream_retries cannot be negative" (fun () ->
      ignore (Openai.Config.make ~max_stream_retries:(-1) ()))
...
let reasoning_effort_encoding () =
  let cases =
    ...
    [ (Disabled, "none"); (High, "high"); (Extra_high, "xhigh"); (Max, "max") ]
  in
  List.iter
    (fun (effort, expected) ->
...
(* The retry budget floors, backoff, reason mapping, and attempt sequence are
   pinned directly against the shared loop in test_llm_http. This smoke proves
   the OpenAI transport wires that loop: a capacity status recovers on a fresh
   attempt and the retry surfaces as an [Event.Retry] carrying the deepened
   capacity limit and its reason. *)
let retry_wiring_recovers_and_announces () =
  let result, requests =
    with_openai_server
      (fun index request ->
        ignore request;
  ...
  in
  let events, _response = expect_stream_ok "retry wiring" result in
  ...
  | [ retry ] ->
      ...
      check "retry delay non-negative" (Llm.Event.Retry.delay retry >= 0.)
  | announced ->
...
let completed_stream_decodes_events_and_response () =
  ...
  | [
   Llm.Event.Text_delta "ok";
   Llm.Event.Reasoning_summary_delta "thought";
   Llm.Event.Usage usage_snapshot;
  ] ->
  ...
  let input_deltas, calls =
    List.fold_left
      (fun (inputs, calls) -> function
        | Llm.Event.Tool_input_delta input -> (input :: inputs, calls)
        | Llm.Event.Tool_call call -> (inputs, call :: calls)
...
let provider_failed_event ?code message =
  let error_fields =
    let fields = [ ("message", Json.string message) ] in
    match code with
    | None -> fields
    | Some code -> ("code", Json.string code) :: fields
  in
  sse_event "response.failed"
    (json_object
       [
         ...
         ( "response",
           json_object
             [
               ("id", Json.string "resp_failed");
               ("status", Json.string "failed");
               ("error", json_object error_fields);
             ] );
       ])

(* A generic mid-stream processing fault carries a 200 response, so the
   pre-stream HTTP retry never sees it; the provider re-runs the whole request
   and rides through to the recovered stream. The default stream budget is the
   deeper codex-style 5, so a single recovery announces limit 5. *)
let stream_provider_error_is_retried () =
  let message =
    "An error occurred while processing your request. Please include the \
     request ID req_1 in your message."
  in
  let result, requests =
    with_openai_server
      (fun index request ->
        ignore request;
        if index = 0 then sse_response [ provider_failed_event message ]
        else sse_response [ text_terminal "recovered" ])
      (fun port ->
        let base_url = "http://127.0.0.1:" ^ string_of_int port in
        let config = Openai.Config.make ~base_url () in
        run_stream ~config port (request ()))
  in
...
let stream_retry_is_disabled_by_max_retries_zero () =
  let result, requests =
    with_openai_server
      (fun index request ->
        ignore index;
        ignore request;
        sse_response [ provider_failed_event "processing error" ])
      (fun port ->
        let base_url = "http://127.0.0.1:" ^ string_of_int port in
        let config = Openai.Config.make ~base_url ~max_retries:0 () in
        run_stream ~config port (request ()))
  in
  ...
  equal_error_kind "disabled stream error kind" Llm.Error.Provider error
...
let stream_error_after_output_is_not_retried () =
      ...
      (fun index request ->
        ...
        sse_response
          [
            ...
            provider_failed_event "processing error";
          ])
      (fun port ->
        let base_url = "http://127.0.0.1:" ^ string_of_int port in
        let config = Openai.Config.make ~base_url () in
  ...
  equal int ~msg:"no retry announced" 0 (List.length (retry_events events));
  equal_error_kind "error after output kind" Llm.Error.Provider error
...
let terminal_stream_error_is_not_retried () =
  let result, requests =
    with_openai_server
      (fun index request ->
        ...
        ignore request;
        sse_response
          [ provider_failed_event ~code:"context_length_exceeded" "too long" ])
      (fun port ->
        let base_url = "http://127.0.0.1:" ^ string_of_int port in
        let config = Openai.Config.make ~base_url () in
        run_stream ~config port (request ()))
  ...
  equal int ~msg:"no retry announced" 0 (List.length (retry_events events));
  equal_error_kind "terminal error kind" Llm.Error.Context_overflow error
...
let () =
  run "mentat.llm.openai"
    [
      ...
      test "retry wiring recovers and announces"
        retry_wiring_recovers_and_announces;
      ...
      test ~timeout:5.0 "stream provider error is retried"
        stream_provider_error_is_retried;
