...
module Google = Mentat_llm_google
...
let with_google_server respond f =
  ...
  in
  let requests_path = Filename.temp_file "mentat-google-requests" ".bin" in
...
let unresolved_references_do_not_touch_transport () =
  ...
  let result, requests =
    with_google_server
      (fun _index _request ->
        failf "unresolved reference must not reach transport")
      (fun port -> run_stream port (request ~transcript ()))
  in
  let events, error = expect_stream_error "unresolved reference" result in
  equal int ~msg:"events" 0 (List.length events);
  equal_error_kind "error kind" Llm.Error.Invalid_request error;
  equal_error_phase "error phase" Llm.Error.Startup error;
...
(* A stream that emits a text part but never reports a finishReason ends as a
   retryable malformed stream, after a delta has already reached the caller. *)
let partial_text_stream text =
  sse_response
    [
      sse_event
        (json_object
...
(* An empty event-stream response opens with a 200 and then ends without a
   finishReason: a retryable malformed stream that surfaced no output, so the
   re-run guard does not block it. *)
let empty_malformed_stream = sse_response []

(* Google now shares the stream re-run combinator (wave 6): a pre-first-token
   stream fault re-runs the whole request, recovering on the next attempt. The
   default stream budget is the deeper codex-style 5. *)
let stream_fault_is_retried () =
  let result, requests =
    with_google_server
      (fun index request ->
        ignore request;
        if index = 0 then empty_malformed_stream else text_stream "recovered")
      (fun port ->
        let base_url = "http://127.0.0.1:" ^ string_of_int port in
        let config = Google.Config.make ~base_url () in
...
(* A retryable stream fault that arrives after a delta reached the caller is
   surfaced, not re-run: replaying a fresh attempt would double the shown text. *)
let stream_fault_after_output_is_not_retried () =
  let result, requests =
    with_google_server
      (fun index request ->
        ignore index;
        ignore request;
        partial_text_stream "partial")
      (fun port ->
        let base_url = "http://127.0.0.1:" ^ string_of_int port in
        let config = Google.Config.make ~base_url () in
        run_stream ~config port (request ()))
  in
  let events, error = expect_stream_error "fault after output" result in
  ...
  equal_error_kind "fault after output kind" Llm.Error.Malformed_stream error
...
let stream_retry_is_disabled_by_max_retries_zero () =
  let result, requests =
    with_google_server
      (fun index request ->
        ignore index;
        ignore request;
        empty_malformed_stream)
      (fun port ->
        let base_url = "http://127.0.0.1:" ^ string_of_int port in
        let config = Google.Config.make ~base_url ~max_retries:0 () in
        run_stream ~config port (request ()))
  in
  ...
  equal int ~msg:"no retry announced" 0 (List.length (retry_events events));
  equal_error_kind "disabled stream error kind" Llm.Error.Malformed_stream error

(* A persistently failing retryable stream fault re-runs only up to the budget
   and then surfaces: the re-run is bounded. This is the sanctioned-delta caveat
   — every retryable stream kind, including the RESOURCE_EXHAUSTED to
   Rate_limited mapping, re-runs the same bounded way; the pre-stream
   terminal-quota fast-fail does not apply to a stream re-run. *)
let stream_retry_is_bounded_by_budget () =
  let result, requests =
    with_google_server
      (fun index request ->
        ignore index;
      ...
      (fun port ->
        let base_url = "http://127.0.0.1:" ^ string_of_int port in
        let config = Google.Config.make ~base_url ~max_stream_retries:2 () in
        run_stream ~config port (request ()))
  in
  let events, error = expect_stream_error "stream retry bounded" result in
  equal int ~msg:"initial attempt plus two re-runs" 3 (List.length requests);
  equal int ~msg:"two retries announced" 2 (List.length (retry_events events));
  equal_error_kind "bounded stream error kind" Llm.Error.Malformed_stream error
  ...
  let text_deltas, calls, usage_events, reasoning =
    List.fold_left
      (fun (texts, calls, usages, reasoning) -> function
        | Llm.Event.Text_delta text -> (text :: texts, calls, usages, reasoning)
        | Llm.Event.Tool_call call -> (texts, call :: calls, usages, reasoning)
        | Llm.Event.Usage usage -> (texts, calls, usage :: usages, reasoning)
        ...
        | Llm.Event.Retry _ -> (texts, calls, usages, reasoning))
      ([], [], [], []) events
  in
...
let textless_reasoning_parts_are_dropped () =
  (* The reported "reasoning parts require text or summary content" error: a
     reasoning part carried over from another provider — Anthropic's signed but
     textless thinking, an OpenAI item with only id + encrypted — has nothing to
     render as a Gemini thought and is dropped, not faulted. A reasoning part
     with text is kept, and its signature rides along as a thoughtSignature. *)
  let anthropic_like =
    Llm.Message.Assistant.Reasoning.make ~signature:"sig_omit" ()
  in
  let openai_like =
    Llm.Message.Assistant.Reasoning.make ~id:"rs_1" ~encrypted:"enc_blob" ()
  in
  let gemini_like =
    Llm.Message.Assistant.Reasoning.make ~text:"a thought"
    ...
    Llm.Transcript.of_list_exn
      [
        ...
        Llm.Message.assistant
          (Llm.Message.Assistant.make
             [
               Llm.Message.Assistant.reasoning_part anthropic_like;
               Llm.Message.Assistant.reasoning_part openai_like;
               Llm.Message.Assistant.reasoning_part gemini_like;
  ...
  let contents = list_field "body" "contents" body in
  let assistant_parts = list_field "assistant" "parts" (List.nth contents 1) in
  equal int ~msg:"textless reasoning parts are dropped" 2
    (List.length assistant_parts);
  let thought = List.nth assistant_parts 0 in
  equal string ~msg:"kept thought text" "a thought"
    (string_field "thought" "text" thought);
  check "kept thought is flagged" (bool_field "thought" "thought" thought);
  equal string ~msg:"thought signature rides along" "thought_sig"
    (string_field "thought" "thoughtSignature" thought);
  equal string ~msg:"visible text follows" "answer"
    (string_field "assistant text" "text" (List.nth assistant_parts 1))
...
let () =
  run "mentat.llm.google"
    [
      test "model, config, and credentials" model_config_and_credentials;
      test "unresolved references do not touch transport"
        unresolved_references_do_not_touch_transport;
      test "maximal request encoding" maximal_request_encoding;
      test "thought signatures round trip" thought_signatures_round_trip;
      test "textless reasoning parts are dropped"
        textless_reasoning_parts_are_dropped;
      ...
      test ~timeout:5.0 "stream fault is retried" stream_fault_is_retried;
      test "stream fault after output is not retried"
