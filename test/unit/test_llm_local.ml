  ...
  List.iter
    (fun entry ->
      ...
      check
        ...
        (match Local.Modelfit.verdict ~budget:max_int fit with
        | Local.Modelfit.Verdict.Fits -> true
        | Local.Modelfit.Verdict.Tight _ | Local.Modelfit.Verdict.Wont_run ->
            false))
    Local.Manifest.all

(* The download URL is a case-sensitive Hugging Face [resolve/main] path; a
   mis-cased artifact filename resolves to HTTP 404. *)
let manifest_download_url () =
  match Local.Manifest.find "gpt-oss-20b" with
  | None -> failf "gpt-oss-20b not in manifest"
  | Some entry ->
      equal string ~msg:"gpt-oss-20b artifact url"
...
let fit_verdicts () =
  ...
  check "fits at 24 GiB"
    ...
    | Local.Modelfit.Verdict.Fits -> true
    | Local.Modelfit.Verdict.Tight _ | Local.Modelfit.Verdict.Wont_run -> false);
  ...
  check "won't run at 8 GiB"
    ...
    | Local.Modelfit.Verdict.Wont_run -> true
    | Local.Modelfit.Verdict.Fits | Local.Modelfit.Verdict.Tight _ -> false);
...
let artifact_status () =
  ...
  | Local.Artifact.Missing { path; url; size } ->
      ...
      check "missing size is exact" (Int64.equal size 12_109_566_624L)
...
let expect_stream_ok msg = function
  | Ok value -> value
  | Error ((_ : Llm.Event.t list), error) ->
...
let unresolved_references_do_not_resolve_models () =
  ...
  in
  match run_stream client request with
  | Ok _ -> failf "expected unresolved reference error"
  | Error (events, error) ->
...
let managed_server_round_trip () =
  ...
  let reasoning =
    List.filter_map
      (function
        | Llm.Event.Reasoning_summary_delta text -> Some text | _ -> None)
      events
  in
  ...
  equal int ~msg:"tool input deltas" 2
    (List.length
       (List.filter
          (function Llm.Event.Tool_input_delta _ -> true | _ -> false)
          events));
...
let readiness_exception_cancellation_reaps_server () =
  ...
  | Error (_, error) ->
      failf "readiness cancellation became an LLM error: %a" Llm.Error.pp error);
...
let term_ignoring_readiness_is_killed () =
  with_fake ~startup_timeout_s:0.05
    ~env:
      [
        ("MENTAT_FAKE_LLAMA_UNHEALTHY", "1");
        ("MENTAT_FAKE_LLAMA_IGNORE_TERM", "1");
      ]
  @@ fun ~dir ~gguf ~config ->
...
(* Which resident server answered is observable through env inheritance: a
   server keeps dumping to the MENTAT_FAKE_LLAMA_DUMP path it was spawned
...
(* Model memory-fit — folded from test_modelfit.ml; now the private Mentat_llm_local.Modelfit. *)
module Modelfit_tests = struct
  module Fit = Local.Modelfit
  ...
  let machine_validation () =
    ...
    expect_invalid_arg "wired limit must be positive" (fun () ->
        Fit.Machine.make ~os:Fit.Machine.Macos ~ram_bytes:gib
          ~wired_limit_bytes:0 ())
  ...
  let estimate_decomposition () =
    ...
    equal int ~msg:"weights pass through" (8 * gib)
      est.Fit.Estimate.weights_bytes;
  ...
  let estimate_kv_dtypes () =
    ...
    in
    equal int ~msg:"q8_0 cache is 34/64 of f16"
      (4 * gib * 17 / 32)
      (kv Fit.Q8_0);
  ...
  let verdict_boundaries () =
    ...
    equal verdict ~msg:"weights alone over budget won't run"
      Fit.Verdict.Wont_run
  ...
  let gguf_per_layer_kv_heads () =
    let kvs =
      ...
      @ [
          (fun b ->
            kv_u32_array b "qwen3moe.attention.head_count_kv" [ 2; 8; 4 ]);
        ]
    in
  ...
  let gguf_stops_before_tokenizer () =
    ...
    let data =
      gguf ~kv_count:8
        (qwen_kvs @ [ (fun b -> b_str b "tokenizer.ggml.tokens") ])
    in
...
let () =
  run "mentat.llm.local"
    (Modelfit_tests.suite
    @ [
        test "manifest integrity" manifest_integrity;
        test "manifest download url" manifest_download_url;
        ...
        test "unresolved references do not resolve models"
          unresolved_references_do_not_resolve_models;
