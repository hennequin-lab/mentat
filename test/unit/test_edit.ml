...
let root = W.Root.of_dir (abs "/workspace")
...
let invalid_targets_are_structured () =
  let parent = path "parent" in
  let symlink =
    Edit.Error.invalid_target ~path:parent Edit.Error.Target_error.Symlink
  in
  let not_a_directory =
    Edit.Error.invalid_target ~path:parent
      Edit.Error.Target_error.Not_a_directory
  in
  equal (option path_value) ~msg:"the offending component is queryable"
    (Some parent) (Edit.Error.path symlink);
  equal string ~msg:"the symlink reason has a diagnostic"
    "symlink targets are not supported"
    (Edit.Error.Target_error.message Edit.Error.Target_error.Symlink);
  is_false ~msg:"distinct target failures do not compare equal"
    (Edit.Error.equal symlink not_a_directory)
...
let diff_renders_all_changes () =
  ...
  let stats = Textdiff.stats diff in
  equal int ~msg:"changed files" 3 stats.Textdiff.files;
  equal int ~msg:"additions" 2 stats.Textdiff.additions;
  equal int ~msg:"deletions" 2 stats.Textdiff.deletions

let default_diff_escapes_header_path () =
  let file = path "bad\nname.txt" in
  let rendered = Edit.diff (rewrite file "old\n" "new\n") in
  match String.split_on_char '\n' (Textdiff.to_string rendered) with
...
let lock_failure_blocks_all_work () =
  ...
  equal phase_value ~msg:"lock failure is preflight"
    Edit.Apply_error.Phase.Preflight
    (Edit.Apply_error.phase error);
...
let accessors_expose_apply_evidence () =
  ...
  equal (option identity_value) ~msg:"observed identity"
    (Some (Mentat_digest.Content_ref.of_contents "old\n"))
...
let duplicate_revalidated_path_blocks_reads_and_writes () =
  ...
  equal phase_value ~msg:"duplicate path is preflight"
    Edit.Apply_error.Phase.Preflight
    (Edit.Apply_error.phase error);
  ...
  List.iter
    (fun (label, plan, targets, expected) ->
      ...
      equal edit_error ~msg:(label ^ " error") expected
        (Edit.Apply_error.error error);
      equal phase_value ~msg:(label ^ " is preflight")
        Edit.Apply_error.Phase.Preflight
        (Edit.Apply_error.phase error);
...
let stale_write_blocks_all_mutation () =
  ...
  equal edit_error ~msg:"stale error"
    ...
    (Edit.Apply_error.error error);
  equal phase_value ~msg:"stale write is preflight"
    Edit.Apply_error.Phase.Preflight
    (Edit.Apply_error.phase error);
...
let revalidation_failure_blocks_reads_and_writes () =
  let file = path "new.txt" in
  let blocked = Edit.Error.out_of_workspace file in
  ...
  equal edit_error ~msg:"revalidation error" blocked
    (Edit.Apply_error.error error);
  equal phase_value ~msg:"revalidation failure is preflight"
    Edit.Apply_error.Phase.Preflight
    (Edit.Apply_error.phase error);
...
let revalidation_outside_workspace_blocks_reads_and_writes () =
  let file = path "new.txt" in
  let outside_root = W.Root.of_dir (abs "/outside") in
  let outside =
    W.Path.make ~root_key:(W.Root.key outside_root) (rel "new.txt")
  in
...
let read_error_blocks_mutation () =
  ...
  equal phase_value ~msg:"read failure is preflight"
    Edit.Apply_error.Phase.Preflight
    (Edit.Apply_error.phase error);
  equal (list result_entry) ~msg:"nothing applied" []
    (Edit.Apply_error.applied error);
...
let later_write_failure_reports_confirmed_entries () =
  ...
  equal edit_error ~msg:"write failure"
    ...
    (Edit.Apply_error.error error);
  equal phase_value ~msg:"stopped committing b"
    (Edit.Apply_error.Phase.Commit { target = b })
    (Edit.Apply_error.phase error);
...
let later_remove_failure_reports_confirmed_entries () =
  ...
  equal edit_error ~msg:"remove failure"
    ...
    (Edit.Apply_error.error error);
  equal phase_value ~msg:"stopped removing b"
    (Edit.Apply_error.Phase.Commit { target = b })
    (Edit.Apply_error.phase error);
  equal target_value ~msg:"a was written" (text "A\n") (current fake a);
...
let first_commit_failure_is_commit_phase_not_preflight () =
  let requested = path "requested.txt" in
  let canonical = path "canonical.txt" in
  let b = path "b.txt" in
  let plan = concat [ rewrite requested "a\n" "A\n"; rewrite b "b\n" "B\n" ] in
  let fake =
    make_io
      ~targets:[ (canonical, text "a\n"); (b, text "b\n") ]
      ~revalidate:(fun path ->
        if W.Path.equal path requested then Ok canonical else Ok path)
      ()
  in
  fake.fail_write <- Some canonical;
  let error =
    expect_apply_error "first commit failure"
      (Edit.apply ~io:fake.io ~workspace plan)
  in
  equal edit_error ~msg:"write failure"
    (Edit.Error.io ~path:canonical "write failed")
    (Edit.Apply_error.error error);
  equal (list result_entry) ~msg:"empty applied prefix" []
    (Edit.Apply_error.applied error);
  equal phase_value ~msg:"empty applied still marks the attempted commit"
    (Edit.Apply_error.Phase.Commit { target = canonical })
    (Edit.Apply_error.phase error);
  equal target_value ~msg:"unreached suffix untouched" (text "b\n")
    (current fake b)
...
let () =
  run "mentat.edit"
    [
      ...
      test "invalid targets are structured" invalid_targets_are_structured;
      ...
      test "first commit failure is commit phase, not preflight"
        first_commit_failure_is_commit_phase_not_preflight;
    ]
