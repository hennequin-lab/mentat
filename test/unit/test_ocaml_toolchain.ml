...
open Windtrap
module Toolchain = Mentat_ocaml_toolchain
...
let explicit_override () =
  ...
  let t =
    discover ~bindings:[ ("PATH", path_bin); ("MENTAT_FAKETOOL", custom) ] ()
  in
  ...
  let broken =
    discover
      ~bindings:
        [ ("PATH", path_bin); ("MENTAT_FAKETOOL", "/no/such/executable") ]
      ()
  in
  equal (option string)
  ...
  is_true ~msg:"the hint names the broken override"
    (contains ~affix:"MENTAT_FAKETOOL is set to /no/such/executable"
...
let local_bin project = Filename.concat (Filename.concat project "_opam") "bin"

let toolchain_env_bridges_the_switch () =
  with_layout @@ fun ~path_bin ~switch ~project ->
  (* dune on PATH, the compiler only in the local switch: [env ~program:"dune"]
     stays unchanged (dune resolves on PATH), but [toolchain_env] must still put
     the switch bin on PATH so the dune spawned in it finds the compiler. *)
  let _ = make_exe path_bin "dune" in
  let _ = make_exe (local_bin project) "ocamlc" in
  let t =
    discover ~bindings:[ ("PATH", path_bin) ] ~workspace_root:project ()
  in
  equal (option string) ~msg:"env for dune is unchanged when dune is on PATH"
    (Some path_bin)
    (env_path (Toolchain.env t ~program:"dune"));
  equal (option string)
    ~msg:"toolchain_env prepends the local switch bin ahead of PATH"
    (Some (local_bin project ^ ":" ^ path_bin))
    (env_path (Toolchain.toolchain_env t));
  let both =
    discover
      ~bindings:[ ("PATH", path_bin); ("OPAM_SWITCH_PREFIX", switch) ]
      ~workspace_root:project ()
  in
  equal (option string) ~msg:"both switch bins precede PATH in ladder order"
    (Some
       (Filename.concat switch "bin" ^ ":" ^ local_bin project ^ ":" ^ path_bin))
    (env_path (Toolchain.toolchain_env both));
  let env = [| "PATH=" ^ path_bin |] in
  let none = Toolchain.discover ~env ~workspace_root:None in
  is_true
    ~msg:"toolchain_env is physically unchanged when no switch is recovered"
    (Toolchain.toolchain_env none == env)
...
let diagnostics_name_the_rungs () =
  ...
  is_true ~msg:"hint keeps the launch-context lead"
    (contains ~affix:"dune is not on Mentat's PATH" hint);
  is_true ~msg:"hint enumerates the override rung"
    (contains ~affix:"MENTAT_DUNE unset" hint);
...
let () =
  run "mentat.ocaml.toolchain"
    [
      ...
      test "toolchain_env bridges the switch" toolchain_env_bridges_the_switch;
